# Operational Index

## Start Here

- [Comece Aqui](comece-aqui.md)
- [Master Guide](master-guide.md)
- [Start Here](start-here.md)
- [Internal Quickstart](internal-quickstart.md)
- [Chat Operating Pattern](chat-operating-pattern.md)
- [Client Suggestion Registry](client-suggestion-registry.md)
- [Client Workspace Update](client-workspace-update.md)
- [Product Master Guide](product-master-guide.md)
- [Backend Architecture Blueprint](backend-architecture.md)
- [API Contract Draft](api-contract.md)
- [Backend Folder Structure](backend-folder-structure.md)
- [Local DB Setup](local-db-setup.md)
- [Migration Order](migration-order.md)
- [Postgres Local README](../../infra/postgres/README.md)
- [Daily Usage Guide](daily-usage-guide.md)
- [Operating Flow](operating-flow.md)
- [Client Intake Guide](client-intake-guide.md)
- [Contextual Market Intelligence](contextual-market-intelligence.md)
- [Social Intelligence Intake](social-intelligence-intake.md)
- [Strategy Intelligence Intake](strategy-intelligence-intake.md)
- [Social Intelligence Summary Template](social-intelligence-summary-template.md)
- [GEO / AI Discoverability Summary Template](geo-discoverability-summary-template.md)
- [Research and Proposal Route](research-and-proposal-route.md)
- [Open Source Social Collection Flow](open-source-social-collection-flow.md)
- [Open Source Social Monitoring Backlog](open-source-social-monitoring-backlog.md)
- [Researcher Open Source Prompt](researcher-open-source-prompt.md)
- [Proposal and Deck Template](proposal-and-deck-template.md)
- [Client Report Template](client-report-template.md)
- [Social Intelligence Step](../steps/step-00-social-intelligence.md)
- [Naming Standard](naming-standard.md)

## Client Context

- Client Record in the database
- Market Research record in the database
- [Client Record Template](client-record-template.md)
- [Example Client Record](example-client-record.md)
- [Intake Sources](intake-sources.md)

## Strategy and Planning

- [Research Record](research-record.md)
- [Domain Framework](domain-framework.md)
- [Campaign State Machine](campaign-state-machine.md)
- [Campaign Planning Gate](campaign-planning-gate.md)
- [Campaign Planning Pack](campaign-planning-pack.md)
- [Campaign Review Pack](campaign-review-pack.md)
- [Stage Contracts](stage-contracts.md)
- [Campaign Backend Map](campaign-backend-map.md)
- [Social Sales Proposal Module](social-sales-proposal-module.md)
- [Weekly Cycle Template](weekly-cycle-template.md)
- [Campaign Transition Workflow](campaign-transition-workflow.md)
- [Next Cycle Briefing](../../output/amiclube/strategy/next-cycle-briefing.md) review copy only
- [YouTube Intelligence Brief Quickstart](youtube-intelligence-brief-quickstart.md)
- [YouTube Intelligence Consolidated Brief Template](youtube-intelligence-consolidated-brief-template.md)
- [YouTube Intelligence Client Adaptation Template](youtube-intelligence-client-adaptation-template.md)
- [Video Intelligence Decision Protocol](video-intelligence-decision-protocol.md)
- [Account Launch Kit](account-launch-kit.md)
- [Content Plan](../../output/strategy/content-plan.md) review copy only
- [Market Intel](../../output/research/market-intel.md) review copy only
- [Blog Brief Template](blog-brief-template.md)
- [Blog Family Ledger Template](blog-family-ledger-template.md)

## Content Production

- [Tone of Voice](tone-of-voice.md)
- [Output Examples](output-examples.md)
- [Creative Playbook](../../output/creative/operational-playbook.md) review copy only
- [Creative Validation Checklist](../../output/creative/validation-checklist.md) review copy only
- [Social Generation Contract](generation-contract.md)
- [Daily Validation Template](../../output/creative/daily-validation-template.md) review copy only
- [Pipeline Compliance Audit Step](../steps/step-04b-pipeline-compliance-audit.md)
- [GEO & AI Discoverability Audit](../../_opensquad/core/best-practices/geo-ai-discoverability.md)
- [Discovery Optimization Guide](content-discovery-optimization.md)
- Remotion Video Skill (`remotion-video-skill`) for motion assets and short-form video
- [Blog Brief](../../output/blog/blog-brief.md) review copy only
- [Blog Draft](../../output/blog/blog-post-draft.md) review copy only
- [Blog Architecture](../../output/blog/blog-architecture.md) review copy only
- [Discovery Optimized Blog](../../output/blog/blog-post.md) review copy only
- [Blog Post](../../output/blog/blog-post.md) review copy only
- [Content Repurpose](../../output/repurposing/content-repurpose.md) review copy only
- [Anti-Patterns](anti-patterns.md)
- [Current Content Production Package](../../output/content/content-production-package.md) review copy only
- [Content Review](../../output/review/content-review.md) review copy only
- [NLM Visual Prototype](nlm-visual-prototype.md)
- [NLM Visual Prototype Prompts](nlm-visual-prototype-prompts.md)
- [NLM Visual Prototype Step](../steps/step-03a2-nlm-visual-prototype.md)

## Publishing

- [Daily Execution Checklist](daily-execution-checklist.md)
- [Git Sync Policy](git-sync-policy.md)
- [Current Creative Playbook](../../output/creative/operational-playbook.md) review copy only
- Current Pipeline Compliance Report: `../../output/{client}/review/pipeline-compliance-{asset-or-batch-id}.md`
- [Current Schedule Approval](../../output/approvals/schedule-approval.md) review copy only
- [Current Schedule Plan](../../output/publishing/schedule-plan.md) review copy only

## Monitoring

- [Monitoring Intake](monitoring-intake.md)
- [Monitoring KPIs](monitoring-kpis.md)
- [Monitoring Rhythm](monitoring-rhythm.md)
- [Action Playbook](action-playbook.md)

## Expansion

- [Future Expansion Roadmap](future-expansion-roadmap.md)
- [Multi-Agency Platform Blueprint](multi-agency-platform.md)
- [Backend Architecture Blueprint](backend-architecture.md)
- [API Contract Draft](api-contract.md)
- [Backend Folder Structure](backend-folder-structure.md)
- [Relational Schema Draft](relational-schema.md)
- [MVP Roadmap](mvp-roadmap.md)
- [Screen Map](screen-map.md)

## Usage Rule

Read the files in this order when working a new client:
1. Database-backed client and product records
2. Client Intake Guide
3. Research Record
4. Research and Proposal Route
5. Open Source Social Collection Flow
6. Researcher Open Source Prompt
7. Optional Social Sales Proposal Module, when needed
8. Content Plan
9. Blog Brief
10. GEO & AI Discoverability Audit, when the site or entity is still ambiguous
11. Blog Architecture
12. Blog Draft
13. Discovery Optimization Guide
14. Discovery Optimized Blog
15. Blog Post
16. Content Repurpose
17. Content Production Package
18. Optional NLM Visual Prototype, only when Atlas decides it reduces visual uncertainty
19. Content Review
20. Schedule Plan
21. Monitoring files
