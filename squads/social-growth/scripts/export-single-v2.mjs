...
if (!assetId || !htmlFile || !outputDir) {
  console.error("Usage: node export-single-v2.mjs --asset AC-30-31 --html <path> --output <dir>");
  console.error("Example: node export-single-v2.mjs --asset AC-30-31 --html output/amiclube/social/previews/ac-30-31-carousel-v2.html --output output/amiclube/social/publish/ac-30-31-v2");
  process.exit(2);
}
...
for (let i = 1; i <= count; i++) {
  const slideNum = i;
  console.log(`  Capturing slide ${slideNum}/${count}...`);
  
  await page.goto(`file://${htmlPath}`, { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(500);
  
  const slideNumEl = await page.$(`.slide-number, .frame-number`);
  if (slideNumEl) {
    await page.evaluate((n) => {
      const el = document.querySelector(`.slide-number, .frame-number`);
  ...
  const outFile = path.join(outPath, `${assetId}-v2-${String(slideNum).padStart(2, "0")}.png`);
