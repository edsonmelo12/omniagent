...
export const queryOne = async <T extends QueryResultRow = QueryResultRow>(text: string, params: unknown[] = []) => {
  const rows = await query<T>(text, params);
  return rows[0] ?? null;
};

export const withTransaction = async <T>(operation: (client: import("pg").PoolClient) => Promise<T>) => {
  const client = await getPool().connect();
