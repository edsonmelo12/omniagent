...
import { findLatestClientRecordByClientId, listClientRecordsByClientId } from "../client-record/client-record.repository.js";
...
import { findLatestBrandProfileByClientId, listBrandProfilesByClientId } from "../brand-profile/brand-profile.repository.js";
import { findPrimaryProductByClientId } from "../client-products/client-products.repository.js";
import {
  findLatestOfferProfileByProductId,
  listOfferProfilesByProductId,
  type OfferProfileRow,
} from "../offer-profile/offer-profile.repository.js";
import { getOfferProfileSignals, type OfferProfileSignals } from "../offer-profile/offer-profile.service.js";
import {
  createCreativeProfileSources,
  createCreativeProfileVersion,
  findLatestCreativeProfileByClientId,
  listCreativeProfileSourcesByCreativeProfileId,
  listCreativeProfilesByClientId,
  updateCreativeProfileVersion,
  type CreativeProfileRow,
} from "./creative-profile.repository.js";
...
const chooseStatusLabel = (confidence: number, evidenceCount: number) => {
  if (confidence >= 80) return "confirmed";
  if (confidence >= 55 || evidenceCount >= 3) return "partial";
  return "needs confirmation";
};

const inferVisualIdentity = (input: { brandVisualStyle: string | null; marketVisualStyle: string | null; tone: string | null; segment: string | null }) => {
...
const inferTypographyDirection = (input: { tone: string | null; visualStyle: string | null; segment: string | null }) => {
...
const inferLayoutFamily = (input: { marketStage: string | null; offerType: string | null; visualStyle: string | null }) => {
...
const inferImageRiskLevel = (input: { proofScore: number | null; brandConfidence: number; offerConfidence: number }) => {
  const proof = input.proofScore ?? 0;
  const confidence = Math.round((input.brandConfidence + input.offerConfidence) / 2);

  if (proof >= 70 && confidence >= 70) return "low";
  if (proof >= 45 || confidence >= 50) return "medium";
  return "high";
};

const inferPrimaryGoal = (input: { presence: number | null; proof: number | null; conversion: number | null; consistency: number | null }) => {
...
const inferMood = (input: { brandVisualStyle: string | null; tone: string | null; segment: string | null; marketStage: string | null }) => {
...
const inferBrandFeel = (input: { brandVisualStyle: string | null; tone: string | null; segment: string | null; marketStage: string | null }) => {
  const source = [input.brandVisualStyle, input.tone, input.segment, input.marketStage].filter(Boolean).join(" ");
  const tokens = lowerTokens(source);
...
const inferOverlayGuidance = (input: { imageRiskLevel: string; mood: string | null; visualIdentity: string | null }) => {
  const source = [input.mood, input.visualIdentity].filter(Boolean).join(" ");
  const tokens = lowerTokens(source);
...
const inferImageRecommendation = (input: { marketStage: string | null; proofScore: number | null; offerType: string | null; segment: string | null }) => {
...
const inferTargetAudience = (input: { audience: string | null; segment: string | null; marketStage: string | null; offerType: string | null }) => {
  if (input.audience) {
...
const inferBuyerPersonas = (input: { audience: string | null; segment: string | null; marketStage: string | null; offerType: string | null }) => {
...
const inferUniqueValueProposition = (input: { positioning: string | null; offerSummary: string | null; marketStage: string | null }) => {
...
const inferBrandArchetype = (input: { tone: string | null; segment: string | null; marketStage: string | null }) => {
...
const inferProductImageryDirection = (input: { marketStage: string | null; proofScore: number | null; offerType: string | null; segment: string | null }) => {
  const source = [input.marketStage, input.offerType, input.segment].filter(Boolean).join(" ");
  const tokens = lowerTokens(source);
...
const inferLogoTreatment = (input: { logoUrl: string | null; brandArchetype: string | null; marketStage: string | null }) => {
  if (!input.logoUrl) {
...
const inferProposalAngle = (input: { uvp: string | null; marketStage: string | null; brandArchetype: string | null }) => {
...
const inferCtaStyle = (input: { marketStage: string | null; offerType: string | null; proofScore: number | null }) => {
  const source = [input.marketStage, input.offerType].filter(Boolean).join(" ");
  const tokens = lowerTokens(source);
...
const buildCreativeSources = (input: {
  client: { website_url: string | null };
  clientRecord: { id: string } | null;
  marketResearch: { id: string } | null;
  socialSnapshot: { id: string } | null;
  brandProfile: { id: string } | null;
  offerProfile: { id: string } | null;
  socialProfiles: Array<{ profile_url: string; platform: string }>;
}) => {
  const sources: Array<{ sourceUrl: string; sourceType: string; confidence: number; notes?: string }> = [];
...
export const buildCreativeProfilePayload = (input: {
  ...
  socialProfiles: Array<{ platform: string; handle: string; profile_url: string; confidence: number }>;
  brandProfile: Awaited<ReturnType<typeof findLatestBrandProfileByClientId>>;
  offerProfileRow: OfferProfileRow | null;
  offerProfile: OfferProfileSignals | null;
  primaryProduct: Awaited<ReturnType<typeof findPrimaryProductByClientId>>;
}) => {
...
const buildCreativeProfileSources = (input: {
  client: { website_url: string | null };
  clientRecord: { id: string } | null;
  marketResearch: { id: string } | null;
  socialSnapshot: { id: string } | null;
...
export const getCreativeProfileSignals = (creativeProfile: CreativeProfileRow | null) => {
  if (!creativeProfile) {
    return null;
  }
...
export const listCreativeProfiles = async (input: { userId: string; agencyId: string; clientId: string }) => {
...
export const getCreativeProfileContext = async (input: { userId: string; agencyId: string; clientId: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
...
export const createCreativeProfile = async (input: { userId: string; agencyId: string; clientId: string; status: string; note?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
...
export const reviseCreativeProfile = async (input: { userId: string; agencyId: string; clientId: string; status: string; note?: string; summaryOverride?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
