...
type SocialPresenceCollectionMode = "default" | "browser";
type BrowserCaptureProvider = (profileUrl: string) => Promise<{
  html: string;
  finalUrl: string;
  title: string;
}>;

export type SocialPresenceObservation = {
  platform: string;
  handle: string | null;
  profileUrl: string;
  ...
  observationStatus: "captured" | "partial" | "unavailable";
  confidence: number;
  notes: string;
...
const parseCountToken = (value: string) => {
...
const extractPatternValue = (source: string, patterns: RegExp[]) => {
...
const extractDateValue = (source: string, patterns: RegExp[]) => {
  for (const pattern of patterns) {
    const match = source.match(pattern);
    if (!match) continue;
...
const buildConfidence = (
  profileConfidence: number | undefined,
...
const buildObservation = (input: {
  ...
  } satisfies SocialPresenceObservation;
};
...
const fetchProfileHtml = async (profileUrl: string, fetchImpl: typeof fetch) => {
...
const collectObservationFromHtml = (
  profile: SocialProfileLike,
  html: string,
  source: "html" | "browser",
  notesSuffix?: string,
  payload?: Record<string, unknown>,
): SocialPresenceObservation => {
  ...
  if (followersMatch) rawMatches.push(followersMatch.matchedPattern);
  if (postsMatch) rawMatches.push(postsMatch.matchedPattern);
  if (latestPostAtMatch) rawMatches.push(latestPostAtMatch.matchedPattern);
  ...
  if (followersCount === null) missing.push("followers");
  if (postsCount === null) missing.push("posts");
  if (latestPostAt === null) missing.push("latest_post_at");
...
const collectBrowserObservation = async (
  profile: SocialProfileLike,
  captureBrowserHtml: BrowserCaptureProvider = fetchProfileHtmlWithBrowser,
): Promise<SocialPresenceObservation> => {
  const normalizedProfileUrl = normalizeProfileUrl(profile.profile_url);
  const normalizedHandle = normalizeHandle(profile.handle);
  const platform = normalizePlatform(profile.platform);
...
const extractYouTubeChannelIdFromUrl = (profileUrl: string) => {
  try {
...
const getYouTubeSearchQueries = (profile: SocialProfileLike) => {
...
const getProfileSearchCandidates = (profile: SocialProfileLike) => {
  const candidates = new Set<string>();
  const normalizedHandle = normalizeHandle(profile.handle);
...
const collectHtmlObservation = async (profile: SocialProfileLike, fetchImpl: typeof fetch): Promise<SocialPresenceObservation> => {
  const platform = normalizePlatform(profile.platform);
  const normalizedHandle = normalizeHandle(profile.handle);
  const normalizedProfileUrl = normalizeProfileUrl(profile.profile_url);
  const patterns = PLATFORM_PATTERNS[platform] ?? PLATFORM_PATTERNS.other;
...
const collectYoutubeApiObservation = async (
  profile: SocialProfileLike,
  fetchImpl: typeof fetch,
): Promise<SocialPresenceObservation | null> => {
  ...
  try {
    ...
    if (!resolvedChannelId) {
      for (const candidate of getYouTubeSearchQueries(profile)) {
        ...
        const itemRecord = firstItem && typeof firstItem === "object" ? (firstItem as Record<string, unknown>) : null;
        const itemId = itemRecord && typeof itemRecord.id === "object" ? (itemRecord.id as Record<string, unknown>) : null;
        const channelId = typeof itemId?.channelId === "string" ? itemId.channelId : null;

        if (channelId) {
          resolvedChannelId = channelId;
          const snippet = itemRecord && typeof itemRecord.snippet === "object" ? (itemRecord.snippet as Record<string, unknown>) : null;
    ...
    const channelRecord = channelItem && typeof channelItem === "object" ? (channelItem as Record<string, unknown>) : null;
    const statistics = channelRecord && typeof channelRecord.statistics === "object" ? (channelRecord.statistics as Record<string, unknown>) : null;
    const snippet = channelRecord && typeof channelRecord.snippet === "object" ? (channelRecord.snippet as Record<string, unknown>) : null;
    ...
    const latestVideoRecord = latestVideo && typeof latestVideo === "object" ? (latestVideo as Record<string, unknown>) : null;
    const latestVideoSnippet = latestVideoRecord && typeof latestVideoRecord.snippet === "object"
      ? (latestVideoRecord.snippet as Record<string, unknown>)
      : null;
    const latestVideoIdRecord = latestVideoRecord && typeof latestVideoRecord.id === "object"
      ? (latestVideoRecord.id as Record<string, unknown>)
      : null;
...
const collectMetaApiObservation = async (
  profile: SocialProfileLike,
  fetchImpl: typeof fetch,
): Promise<SocialPresenceObservation | null> => {
  ...
  try {
    if (platform === "facebook") {
      let pageRecord: Record<string, unknown> | null = null;
      let searchQuery: string | null = null;
      ...
      for (const candidate of getProfileSearchCandidates(profile)) {
        ...
        searchUrl.searchParams.set(
          "fields",
          "id,name,username,link,fan_count,followers_count,posts.limit(1).summary(true){created_time}",
        );
        searchUrl.searchParams.set("access_token", apiKey);

        const searchData = await fetchJson(searchUrl.toString(), fetchImpl);
        const firstItem = Array.isArray(searchData?.data) ? searchData.data[0] : null;
        pageRecord = firstItem && typeof firstItem === "object" ? (firstItem as Record<string, unknown>) : null;
      ...
      const postsRecord = pageRecord.posts && typeof pageRecord.posts === "object" ? (pageRecord.posts as Record<string, unknown>) : null;
      const postsSummary = postsRecord?.summary && typeof postsRecord.summary === "object" ? (postsRecord.summary as Record<string, unknown>) : null;
      const postsData = Array.isArray(postsRecord?.data) ? postsRecord?.data : [];
      const latestPostRecord =
        postsData.length > 0 && typeof postsData[0] === "object" ? (postsData[0] as Record<string, unknown>) : null;

      const followersCount =
        typeof pageRecord.followers_count === "number"
    ...
    discoveryUrl.searchParams.set(
      "fields",
      `business_discovery.username(${username}){username,name,followers_count,media_count,media.limit(1){timestamp,permalink}}`,
    );
    discoveryUrl.searchParams.set("access_token", apiKey);

    const discoveryData = await fetchJson(discoveryUrl.toString(), fetchImpl);
    const discoveryRecord = discoveryData && typeof discoveryData === "object" ? (discoveryData as Record<string, unknown>) : null;
    const businessDiscovery =
      discoveryRecord?.business_discovery && typeof discoveryRecord.business_discovery === "object"
        ? (discoveryRecord.business_discovery as Record<string, unknown>)
        : null;
    ...
    const mediaRecord = businessDiscovery.media && typeof businessDiscovery.media === "object" ? (businessDiscovery.media as Record<string, unknown>) : null;
    const mediaData = Array.isArray(mediaRecord?.data) ? mediaRecord.data : [];
    const latestMediaRecord =
      mediaData.length > 0 && typeof mediaData[0] === "object" ? (mediaData[0] as Record<string, unknown>) : null;
...
const mergeObservations = (primary: SocialPresenceObservation, fallback: SocialPresenceObservation) => {
...
const resolveProfileObservation = async (
  profile: SocialProfileLike,
  fetchImpl: typeof fetch,
  mode: SocialPresenceCollectionMode,
  captureBrowserHtml: BrowserCaptureProvider,
) => {
  const platform = normalizePlatform(profile.platform);
...
const dedupeProfiles = (profiles: SocialProfileLike[]) => {
  const seen = new Set<string>();
  const unique: SocialProfileLike[] = [];
...
export const collectSocialPresenceObservations = async (input: {
  ...
  const observations = await Promise.all(
    uniqueProfiles.map((profile) => resolveProfileObservation(profile, fetchImpl, mode, captureBrowserHtml)),
  );
