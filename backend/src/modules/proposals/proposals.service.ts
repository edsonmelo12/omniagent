...
const chooseArchetype = (
  snapshot: { presence_score: number | null; consistency_score: number | null; proof_score: number | null; conversion_readiness: number | null } | null,
  profileCount: number,
  marketResearch: { payload_json: unknown } | null,
) => {
  if (!snapshot) {
    return "presenca";
  }

  const payload = marketResearch && typeof marketResearch.payload_json === "object" && marketResearch.payload_json !== null ? (marketResearch.payload_json as Record<string, unknown>) : null;
  const marketStage =
    payload?.market && typeof payload.market === "object" && payload.market !== null && typeof (payload.market as Record<string, unknown>).stage === "string"
      ? String((payload.market as Record<string, unknown>).stage)
      : null;
...
const buildPresentationObjective = (archetype: string) => {
  switch (archetype) {
    case "escala":
...
const summarizeSnapshot = (snapshot: {
  ...
  const gaps = Array.isArray(snapshot.main_gaps_json)
    ? snapshot.main_gaps_json
        .map((gap) => {
          if (gap === "presence") return "presenca";
          if (gap === "consistency") return "consistencia";
          if (gap === "proof") return "prova";
          if (gap === "conversion") return "conversao";
          return String(gap);
        })
        .join(", ")
    : "nenhuma";
  ...
  const readinessBand =
    maturity >= 80 ? "forte" : maturity >= 60 ? "boa" : maturity >= 40 ? "base" : "inicial";
...
const summarizeClientRecord = (clientRecord: { payload_json: unknown } | null) => {
  ...
  const payload = clientRecord.payload_json as { narrative?: { summary?: string } };
  return payload.narrative?.summary ?? "Registro do cliente pronto.";
};

const summarizeMarketResearch = (marketResearch: { payload_json: unknown } | null) => {
  ...
  const market = payload.market && typeof payload.market === "object" && payload.market !== null ? (payload.market as Record<string, unknown>) : {};
  const competition = payload.competition && typeof payload.competition === "object" && payload.competition !== null ? (payload.competition as Record<string, unknown>) : {};
...
export const buildProposalCommercialBrief = (input: {
  client: { name: string; segment: string | null; website_url: string | null };
  clientRecord: { payload_json: unknown } | null;
  marketResearch: { payload_json: unknown } | null;
  snapshot: { proof_score: number | null; consistency_score: number | null; conversion_readiness: number | null } | null;
  creativeProfile: ReturnType<typeof getCreativeProfileSignals> | null;
  offerProfile: ReturnType<typeof getOfferProfileSignals> | null;
  brandProfile: Awaited<ReturnType<typeof findLatestBrandProfileByClientId>> | null;
  primaryProduct: Awaited<ReturnType<typeof findPrimaryProductByClientId>> | null;
}) => {
  const clientRecordPayload =
    input.clientRecord && typeof input.clientRecord.payload_json === "object" && input.clientRecord.payload_json !== null
      ? (input.clientRecord.payload_json as Record<string, unknown>)
      : null;
  const marketResearchPayload =
    input.marketResearch && typeof input.marketResearch.payload_json === "object" && input.marketResearch.payload_json !== null
      ...
      : null;
  const narrative = clientRecordPayload?.narrative && typeof clientRecordPayload.narrative === "object" ? (clientRecordPayload.narrative as Record<string, unknown>) : null;
  const diagnosis = clientRecordPayload?.diagnosis && typeof clientRecordPayload.diagnosis === "object" ? (clientRecordPayload.diagnosis as Record<string, unknown>) : null;
  const market = marketResearchPayload?.market && typeof marketResearchPayload.market === "object" ? (marketResearchPayload.market as Record<string, unknown>) : null;
  const targetAudience =
    input.creativeProfile?.targetAudience ??
  ...
  const uniqueValueProposition =
    input.creativeProfile?.uniqueValueProposition ??
  ...
  const objectionsToAddress =
    input.creativeProfile?.proofPoints?.length && input.creativeProfile.proofPoints.length > 0
...
export const getProposalContext = async (input: { userId: string; agencyId: string; clientId: string }) => {
...
export const createProposal = async (input: { userId: string; agencyId: string; clientId: string; status: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
...
export const reviseProposal = async (input: { userId: string; agencyId: string; clientId: string; status: string; thesisOverride?: string; note?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
  ...
  const currentThesis = input.thesisOverride ?? latestProposal[0]?.thesis ?? (
    archetype === "escala"
      ? "Escalar a base social atual com um motor de conteudo repetivel e um fluxo de conversao mais forte."
      : archetype === "conversao"
        ? "Transformar atencao em fluxo de leads alinhando conteudo, CTA e narrativa comercial."
