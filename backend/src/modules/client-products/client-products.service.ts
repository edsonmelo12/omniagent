...
import {
  ...
  updateProductRecord,
} from "./client-products.repository.js";
...
const toProductPayload = (product: Awaited<ReturnType<typeof listProductsByClientId>>[number]) => ({
  ...product,
  proof_points: Array.isArray(product.proof_points_json) ? product.proof_points_json.map(String) : [],
});

export const getProductsContext = async (input: { userId: string; agencyId: string; clientId: string }) => {
