...
import type { ProductRow } from "../client-products/client-products.repository.js";
import { findProductById } from "../client-products/client-products.repository.js";
import {
  createOfferProfileVersion,
  findLatestOfferProfileByClientId,
  findLatestOfferProfileByProductId,
  listOfferProfilesByClientId,
  listOfferProfilesByProductId,
  type OfferProfileRow,
} from "./offer-profile.repository.js";
...
const inferPrimaryCta = (input: { product: ProductRow; brandProfile: BrandProfileSignals | null }) => {
  const source = [
...
const inferTone = (input: { product: ProductRow; brandProfile: BrandProfileSignals | null }) => {
...
const inferProofGaps = (proofPoints: string[]) => {
  ...
  if (hasTestimonialsEvidence) proofGaps.delete("testimonials");
  if (hasReviewsEvidence) proofGaps.delete("reviews");
  if (hasCasesEvidence) proofGaps.delete("cases");
  if (hasBeforeAfterEvidence) proofGaps.delete("before-after");
  if (hasProcessEvidence) proofGaps.delete("process proof");

  return Array.from(proofGaps);
};

const getProductSignalText = (product: ProductRow) =>
  [product.name, product.promise, product.problem_solved, product.audience, product.category, product.offer_type]
    .filter(Boolean)
    .join(" ");

const normalizeProfileStatus = (productStatus: string) => {
  if (productStatus === "draft") return "draft";
  if (productStatus === "validated") return "review";
  if (productStatus === "archived") return "draft";
  return "approved";
};

export type OfferProfileSignals = {
  id: string;
  version: number;
  status: string;
  confidence: number;
...
export const getOfferProfileSignals = (
  offerProfile: OfferProfileRow | null,
): OfferProfileSignals | null => {
  if (!offerProfile) {
    return null;
  }
...
export const getOfferProfilesContext = async (input: { userId: string; agencyId: string; clientId: string }) => {
...
export const getOfferProfileForProduct = async (input: { userId: string; agencyId: string; productId: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
