...
const buildWeights = (marketPayload: PayloadRecord | null) => {
...
type BrandSocialProfileSnapshot = {
  platform: string;
  handle: string | null;
...
const extractBrandSnapshot = (brandProfileRow: Awaited<ReturnType<typeof findLatestBrandProfileByClientId>> | null): BrandSnapshot | null => {
  if (!brandProfileRow) {
    return null;
  }
  ...
  const socialProfiles = asArray(confirmed?.socialProfiles)
    ...
    .filter((value): value is PayloadRecord => Boolean(value))
    .map<BrandSocialProfileSnapshot>((value) => ({
  ...
  return {
    ...
    visualPalette,
    socialProfiles,
    summary: asString((brandProfileRow.confirmed_json as PayloadRecord | null)?.summary) ?? asString((brandProfileRow.inferred_json as PayloadRecord | null)?.summary) ?? null,
  };
};

const extractOfferSnapshot = (offerProfile: ReturnType<typeof getOfferProfileSignals> | null, primaryProduct: Awaited<ReturnType<typeof findPrimaryProductByClientId>>): OfferSnapshot | null => {
  if (!offerProfile) {
    return null;
  }
...
const extractClientBriefSnapshot = (clientRecordRow: Awaited<ReturnType<typeof findLatestClientRecordByClientId>> | null, client: { website_url: string | null; segment: string | null; status: string }) => {
  if (!clientRecordRow) {
    return null;
  }
  ...
  } satisfies ClientBriefSnapshot;
};

const extractSocialProfileSnapshots = (profiles: Array<{ platform: string; handle: string; profile_url: string; status: string; confidence: number }>): SocialProfileSnapshot[] =>
...
export const extractPublicSignals = (marketResearchPayload: PayloadRecord | null) => {
...
const calculateMaturity = (presence: number | null, consistency: number | null, proof: number | null, conversion: number | null) => {
...
const summarizeGrowthMetrics = (monitoringPayload: PayloadRecord | null) => {
...
export const buildClientReportPayload = (input: {
  ...
  marketPresence: Awaited<ReturnType<typeof getMarketPresenceContext>>;
  socialPresence: Awaited<ReturnType<typeof getSocialPresenceContext>>;
  ...
  offerProfile: ReturnType<typeof getOfferProfileSignals>;
  offerProfiles: Awaited<ReturnType<typeof listOfferProfilesByProductId>>;
  creativeProfile: ReturnType<typeof getCreativeProfileSignals> | null;
  creativeProfiles?: Awaited<ReturnType<typeof listCreativeProfilesByClientId>>;
}) => {
  ...
  const clientSummary =
    getRecordString(clientRecordPayload, ["narrative", "summary"]) ??
  ...
  const marketSummary =
    getRecordString(marketResearchPayload, ["market", "summary"]) ??
  ...
  const competitionSummary =
    getRecordString(marketResearchPayload, ["competition", "benchmarkSummary"]) ??
  ...
  const baselineMaturity = calculateMaturity(
    input.marketPresence.latestBaseline?.presence_score ?? null,
    input.marketPresence.latestBaseline?.consistency_score ?? null,
    input.marketPresence.latestBaseline?.proof_score ?? null,
    input.marketPresence.latestBaseline?.conversion_score ?? null,
  );
  ...
  const beforeAfterSummary =
    input.marketPresence.latestComparison
  ...
  const operationalSummary =
    monitoringInterpretations.length > 0
      ? monitoringInterpretations.join(" · ")
      : growth.summary;

  const proposalObjective =
    getRecordString(proposalPayload, ["presentationHeader", "objective"]) ??
  ...
  const brandSummary =
    asString((brandProfilePayload as PayloadRecord).summary) ??
    (input.brandProfile
...
export const getClientReportContext = async (input: { userId: string; agencyId: string; clientId: string }) => {
