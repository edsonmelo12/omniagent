...
import {
  ...
  createMonitoringReportVersion,
  findLatestMonitoringReportByClientId,
  listMonitoringReportsByClientId,
  getFallbackPeriod,
...
const getSchedulePayload = (schedule: { payload_json: unknown; version: number; id: string } | null) => {
  if (!schedule || typeof schedule.payload_json !== "object" || schedule.payload_json === null) {
    return null;
  }

  return schedule.payload_json as {
...
export const getMonitoringContext = async (input: { userId: string; agencyId: string; clientId: string }) => {
...
export const createMonitoringReport = async (input: { userId: string; agencyId: string; clientId: string; status: string; periodStart?: string; periodEnd?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
  ...
  const payload = createMonitoringPayload({
    ...
    editorialPautas: editorialPautas.filter((pauta): pauta is NonNullable<typeof pauta> => typeof pauta?.title === "string").map((pauta, index) => ({
    ...
    scheduleItems: scheduleItems.filter((item): item is NonNullable<typeof item> => typeof item?.title === "string").map((item) => ({
...
export const reviseMonitoringReport = async (input: { userId: string; agencyId: string; clientId: string; status: string; periodStart?: string; periodEnd?: string; note?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
