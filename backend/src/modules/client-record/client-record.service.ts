...
import {
  createClientRecordVersion,
  findLatestClientRecordByClientId,
  listClientRecordsByClientId,
  updateClientRecordVersion,
} from "./client-record.repository.js";
...
const chooseArchetype = (snapshot: { presence_score: number | null; consistency_score: number | null; proof_score: number | null; conversion_readiness: number | null } | null, profilesCount: number) => {
  if (!snapshot) {
    return "presenca";
  }
...
const getOfferProfileContext = async (clientId: string) => {
...
const buildStrategyRecommendationContext = (
  archetype: string,
  input: {
    client: {
      name: string;
  ...
  const recommendations = researchPayload?.recommendations && typeof researchPayload.recommendations === "object" && researchPayload.recommendations !== null
    ? (researchPayload.recommendations as Record<string, unknown>)
    : null;
  ...
  const editorialPillars = Array.isArray(recommendations?.editorialPillars)
    ? (recommendations.editorialPillars as unknown[]).map((item) => (typeof item === "string" ? item : "")).filter(Boolean)
    : [];
...
const buildClientRecordPayload = (input: {
  ...
  const researchSummary =
    researchPayload?.market && typeof researchPayload.market === "object" && researchPayload.market !== null && typeof (researchPayload.market as Record<string, unknown>).summary === "string"
      ? ((researchPayload.market as Record<string, unknown>).summary as string)
      : null;
  const researchPositioning =
    researchPayload?.market && typeof researchPayload.market === "object" && researchPayload.market !== null && typeof (researchPayload.market as Record<string, unknown>).positioning === "string"
      ? ((researchPayload.market as Record<string, unknown>).positioning as string)
      : null;
  const researchWeights =
    researchPayload?.market && typeof researchPayload.market === "object" && researchPayload.market !== null && typeof (researchPayload.market as Record<string, unknown>).weights === "object"
      ? ((researchPayload.market as Record<string, unknown>).weights as Record<string, unknown>)
      : null;
  const researchCompetitors =
    researchPayload?.competition && typeof researchPayload.competition === "object" && researchPayload.competition !== null && Array.isArray((researchPayload.competition as Record<string, unknown>).competitorUrls)
      ? ((researchPayload.competition as Record<string, unknown>).competitorUrls as string[])
      : [];
  const researchBenchmarkSummary =
    researchPayload?.competition && typeof researchPayload.competition === "object" && researchPayload.competition !== null && typeof (researchPayload.competition as Record<string, unknown>).benchmarkSummary === "string"
      ? ((researchPayload.competition as Record<string, unknown>).benchmarkSummary as string)
      : null;
...
export const getClientRecord = async (input: { userId: string; agencyId: string; clientId: string }) => {
...
export const createClientRecord = async (input: { userId: string; agencyId: string; clientId: string; status: string; note?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
...
export const reviseClientRecord = async (input: { userId: string; agencyId: string; clientId: string; status: string; note?: string; summaryOverride?: string }) => {
  await requireAgencyAccess(input.userId, input.agencyId);
  const client = await findClientById(input.agencyId, input.clientId);
  ...
  const narrative = payload.narrative && typeof payload.narrative === "object" ? (payload.narrative as Record<string, unknown>) : {};
  nextPayload.narrative = {
    ...narrative,
    summary: input.summaryOverride ?? (narrative.summary as string | undefined) ?? "Client record revised.",
  };
