...
import type { DiscoveredProfileInput } from "./social-discovery.schemas.js";
...
const collectSocialLinksFromHtml = (html: string) => {
  const matches = html.matchAll(/href="([^"]+)"/g);
  const discovered: Array<{ platform: string; handle: string; profileUrl: string }> = [];
...
export const discoverSocialProfiles = async (input: {
  ...
  clientId: string;
  sourceUrls?: string[];
  discoveredProfiles?: DiscoveredProfileInput[];
  ...
  const sourceCandidates = new Set<string>(input.sourceUrls ?? []);

  if (client.website_url) {
    sourceCandidates.add(client.website_url);
  }

  const clientSites = await queryOne<{ urls: string[] }>(
    `select coalesce(array_agg(url), '{}'::text[]) as urls
     from client_sites
     where client_id = $1`,
    [input.clientId],
  );
  ...
  const discoveredFromSources: Array<{ platform: string; handle: string; profileUrl: string; discoverySource: string; confidence: number }> = [];
