...
const formatList = (items, emptyLabel = "nenhum") => (items.length > 0 ? items.join(" · ") : emptyLabel);
const getArtifactStatus = (value) => {
...
const getContentRhythm = (contentPlan) => {
...
const getEditorialPautas = (contentPlan) => {
    const payload = asRecord(contentPlan?.payload_json);
...
const getMonitoringObservations = (monitoring) => {
...
const compactList = (items, limit = 2) => items.slice(0, limit);
const buildCampaignRepairSuggestion = (blockers, campaignStage) => {
...
const buildFollowUpPrompts = (input) => {
    const { campaignStage, questionFocus, missingArtifacts, pendingApprovals, contentRhythm, editorialPautas, scheduleItems, monitoringObservations, hasClientRecord, hasProposal, hasContentPlan, hasSchedule, hasMonitoring, } = input;
...
const detectFocus = (text) => {
    ...
    if (normalized.includes("gargalo") || normalized.includes("bloqueio") || normalized.includes("trav"))
        return "bottleneck";
    ...
    if (normalized.includes("aprova"))
        return "approval";
    if (normalized.includes("estrat"))
        return "strategy";
    if (normalized.includes("próximo") || normalized.includes("proximo") || normalized.includes("agora") || normalized.includes("falta"))
        return "next";
    if (normalized.includes("risco"))
        return "risk";
    return "general";
};
const buildReasonedConsultation = (question, context, history) => {
    ...
    if (!context.clientRecord)
        operationalBlockers.push("falta client record aprovado");
    if (!context.contentPlan)
        operationalBlockers.push("falta plano de conteúdo");
    if (!context.schedule)
        operationalBlockers.push("falta agenda operacional");
    if (pendingApprovals > 0)
        operationalBlockers.push(`${pendingApprovals} aprovação(ões) pendente(s)`);
    if (!context.monitoring)
        operationalBlockers.push("monitoramento ainda não foi fechado");
    const sourceTrail = [
    ...
    const threadSummary = history.length > 0
    ...
    switch (questionFocus) {
        case "approval":
            ...
            recommendations.push(pendingApprovals > 0 ? "Priorize a fila de aprovação mais antiga antes de expandir a cadência." : "Mantenha a revisão leve e avance para produção.", context.schedule ? "Use a agenda operacional como trilho principal para não perder ritmo." : "Feche a agenda operacional antes de escalar produção.");
            risks.push(pendingApprovals > 0 ? "Pendências podem atrasar publicação e revisão." : "O risco principal é sair da cadência e perder consistência.");
            break;
        case "publish":
            ...
            recommendations.push(context.schedule ? "Publique seguindo a agenda atual e mantenha o mesmo ritmo por pelo menos um ciclo." : "Feche a agenda operacional antes de tentar publicar em volume.", editorialPautas.length > 0 ? `Use as pautas prioritárias: ${formatList(editorialPautas, "nenhuma pauta")}.` : "Crie pautas editoriais a partir do client record e da proposta.");
            risks.push(context.schedule ? "Se a agenda virar só fila, a consistência de mensagem cai." : "Sem agenda, a execução vira improviso.");
            break;
        case "bottleneck":
            ...
            recommendations.push(missingArtifacts.length > 0 ? `Feche primeiro os artefatos ausentes: ${formatList(missingArtifacts, "nenhum")}.` : "Siga para a calibração fina do conteúdo e da distribuição.", pendingApprovals > 0 ? "Liberar a governança reduz a latência do ciclo." : "Mantenha a produção em lotes pequenos para preservar o padrão.");
            risks.push(missingArtifacts.length > 0 ? "Pular etapa faz o sistema perder coerência entre diagnóstico e execução." : "A pressão operacional pode esconder um desvio de estratégia.");
            break;
        case "strategy":
            answer =
            ...
            recommendations.push(context.proposal ? "Use a tese da proposta como eixo das próximas pautas." : "Revise a proposta antes de avançar em volume de conteúdo.", context.clientRecord ? "Conecte os temas editoriais aos problemas que o client record já mapeou." : "Feche o client record para ganhar direção estratégica.");
            risks.push(context.proposal ? "Sem disciplina de pauta, a estratégia vira só discurso." : "Sem tese, o conteúdo tende a ficar reativo.");
            break;
        case "organic":
            answer = `Início do orgânico: ${organicLaunch.readiness}. ${organicLaunch.reason}. Próximo passo: ${organicLaunch.nextStep}.`;
            recommendations.push(organicLaunch.readiness === "bloqueado"
                ? organicLaunch.nextStep
                : "Mantenha o produto foco e feche a base antes de aumentar a cadência.", organicLaunch.readiness === "pronto"
                ? "Abra o primeiro ciclo com cadência simples e repetível."
                : "Feche research, strategy e content plan para sair da preparação.");
            risks.push(organicLaunch.readiness === "bloqueado"
        ...
        case "next":
            ...
            recommendations.push(context.schedule ? "Execute a agenda atual sem alterar o foco do ciclo." : "Finalize os artefatos-base antes de abrir a frente de publicação.", pendingApprovals > 0 ? "Resolva as aprovações para liberar a fila de execução." : "Depois disso, parta para monitoramento e aprendizagem.");
        ...
        case "risk":
            ...
            recommendations.push(monitoringObservations.length > 0 ? "Trate as interpretações de monitoramento como backlog de correção." : "Monitore se a execução está respeitando a proposta e o client record.", pendingApprovals > 0 ? "Acompanhe as aprovações diariamente até zerar a fila." : "Reforce a cadência antes de aumentar a ambição do ciclo.");
        ...
        default:
            ...
            recommendations.push(context.monitoring ? "Proteja a cadência atual e observe a resposta do público." : "Complete os artefatos que faltam para a visão ficar fechada.", context.contentPlan ? "Ajuste apenas o necessário no plano, sem reabrir o ciclo inteiro." : "Feche o planejamento antes de expandir o escopo.");
            risks.push(context.monitoring ? "Adicionar mais frentes agora pode diluir foco." : "A leitura ficará incompleta enquanto faltar contexto estrutural.");
            break;
    }
    ...
    const publicSignalsLine = marketResearchPublicSignals.sourceCount > 0
    ...
    return {
        answer,
        summary: `${activeProduct ? `Produto foco: ${activeProduct.name}. ` : ""}` +
