...
const getArtifactStatus = (value) => (typeof value === "string" ? value : null);
const isApproved = (status) => status === "approved";
const appendUniqueStages = (target, stages) => {
...
const stableSerialize = (value) => JSON.stringify(normalizeForComparison(value));
const getClientRecordPayload = (value) => {
...
const getClientRecordChangedSections = (previous, current) => {
...
const getClientRecordChangeScope = (changedSections) => {
...
const classifyClientRecordPathImpact = (path) => {
...
const buildClientRecordChangeDetails = (changedPaths) => changedPaths.map((path) => ({
    path,
...
const isStaleComparedTo = (sourceVersion, latestVersion) => {
    if (sourceVersion === null || latestVersion === null) {
        return false;
    }
    return sourceVersion < latestVersion;
};
const getCampaignContext = async (input) => {
    ...
    const [profiles, snapshot, clientRecords, marketResearch, proposal, contentPlan, contentPackage, schedule, publishingExecution, monitoring, approvals] = await Promise.all([
...
export const resolveCampaignState = (context) => {
    ...
    const contentPlanStale = clientRecordImpactsStrategy && isStaleComparedTo(contentPlanClientRecordVersion, latestClientRecordVersion) ||
    ...
    const monitoringStale = (clientRecordHasMeaningfulChange && isStaleComparedTo(monitoringClientRecordVersion, latestClientRecordVersion)) ||
    ...
    if (hasPublishingExecution && publishExecutionStatus === "published") {
        ...
        blockers.push(publishExecutionStatus === "blocked"
...
export const getCampaignState = async (input) => {
...
export const assertCampaignStage = async (input) => {
    const context = await getCampaignContext(input);
    const state = resolveCampaignState(context);
    if (!input.allowedStages.includes(state.stage)) {
        throw createError("FAILED_PRECONDITION", `Campaign is currently at ${state.stage}. Earliest allowed stage is ${input.allowedStages.join(", ")}`, 422);
    }
    return state;
};
