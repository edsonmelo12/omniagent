...
import { findProductById, listProductsByClientId } from "../client-products/client-products.repository.js";
import { requireAgencyAccess } from "../tenancy/tenancy.service.js";
import { createStrategyIntelligenceVersion, findLatestStrategyIntelligenceVersionByClientId, findStrategyIntelligenceAssetById, listAllStrategyIntelligenceAssets, listStrategyIntelligenceAssetsByClientId, listLatestStrategyIntelligenceAssetsByClientId, listStrategyLibraryStrategies, syncStrategyLibraryFromAssets, } from "./strategy-intelligence.repository.js";
import { findYoutubeStrategyAnalysisById } from "../youtube-strategy/youtube-strategy.repository.js";
const CAMPAIGN_VARIANT_GUIDANCE = {
    awareness: {
...
const ORGANIC_STRATEGY_CATALOG = [
    {
        id: "authority_proof",
...
const KINDS = ["primary", "alternative", "plan_b"];
const compactText = (value) => value.replace(/\s+/g, " ").trim();
const toList = (value) => Array.isArray(value) ? value.map((item) => (typeof item === "string" ? compactText(item) : "")).filter(Boolean) : [];
...
const buildStrategySourceVideo = (value) => {
...
const buildStrategySourceMetadata = (value) => {
    if (!value || typeof value !== "object")
        return null;
    const source = value;
    const videos = Array.isArray(source.videos)
        ? source.videos.map((item) => buildStrategySourceVideo(item)).filter(Boolean)
        : [];
...
const getStrategySourceVideoIdFromUrl = (value) => {
    const trimmed = value.trim();
    if (!trimmed)
        return null;
    try {
...
const buildStrategySourceVideos = (videos) => {
    const seen = new Set();
...
const splitTokens = (value) => compactText(value)
    .toLowerCase()
    .split(/[^a-z0-9áàâãéèêíìîóòôõúùûç]+/i)
    .map((token) => token.trim())
    .filter((token) => token.length >= 4);
const collectText = (values) => values.filter(Boolean).join(" ");
const countTextOverlap = (left, right) => {
...
const inferStrategyAngles = (strategy) => {
    const context = collectText([strategy.name, strategy.summary, strategy.objective ?? "", ...strategy.evidence]).toLowerCase();
    const angles = new Set();
    if (/autoridade|posicionamento|prova|credibilidade|case/.test(context))
        angles.add("authority");
    if (/educa|ensina|explica|didatico|aula|framework/.test(context))
        angles.add("education");
    if (/convers|lead|captura|funil|cta|conversao/.test(context))
        angles.add("conversion");
    if (/risco|seguranca|fallback|conservador|valid/.test(context))
        angles.add("risk");
    if (/escala|volume|processo|sistema|automat/.test(context))
        angles.add("scale");
    return angles;
};
const inferOfferProfile = (product) => {
...
export const STRATEGY_THEME_LABELS = {
...
export const buildFallbackStrategyName = (kind) => {
    if (kind === "primary") {
...
export const buildFallbackStrategySummary = (kind) => {
    if (kind === "primary") {
...
const inferStrategyTheme = (kind, strategy, product) => {
    if (kind === "plan_b") {
        return "fallback";
    }
    const context = collectText([strategy.name, strategy.summary, strategy.objective ?? "", ...strategy.evidence]).toLowerCase();
...
const scoreProductFit = (strategy, product) => {
    if (!product) {
...
export const buildStrategySummary = (kind, payload, index) => {
    ...
    const summarySource = kind === "primary"
    ...
    const strategyName = kind === "primary"
...
const normalizeSearchText = (value) => value
    ...
    .toLowerCase();
const buildProductFitTags = (product) => {
    const tags = new Set();
    const offerProfile = inferOfferProfile(product);
    const productText = normalizeSearchText(collectText([
        product?.name,
        product?.category,
        product?.offer_type,
        product?.promise,
    ...
    if (offerProfile.isConsultative)
        tags.add("consultive");
    if (offerProfile.isEducational)
        tags.add("educational");
    if (offerProfile.isProductized)
        tags.add("productized");
    if (offerProfile.isHighTicket)
        tags.add("high_ticket");
    if (offerProfile.hasPerformanceNeed)
        tags.add("performance");
    if (/b2b|corporativo|gestor|diretor|empresa/.test(productText))
        tags.add("b2b");
    if (/local|loja|restaurante|clinica|clínica|presencial|whatsapp/.test(productText))
        tags.add("local_business");
    if (/e ?commerce|loja online|checkout|carrinho|frete/.test(productText))
        tags.add("ecommerce");
    if (/creator|youtube|reels|shorts|instagram|podcast/.test(productText))
        tags.add("creator_business");
    return tags;
};
const buildClientProfileTags = (input) => {
    const tags = new Set();
    const clientText = normalizeSearchText(collectText([input.clientSegment, input.productContext, input.serviceContext, input.audienceContext]));
    if (/consult|servi[cç]o|agency|ag[eê]ncia|mentoria|consultoria/.test(clientText))
        tags.add("consultive");
    if (/curso|treinamento|aula|workshop|formacao|formação|educacao|educação/.test(clientText))
        tags.add("educational");
    if (/saas|software|app|plataforma|tech|produto digital/.test(clientText))
        tags.add("productized");
    if (/premium|enterprise|executivo|ticket alto|high ticket/.test(clientText))
        tags.add("high_ticket");
    if (/lead|venda|funil|pipeline|roi|mql|demanda|convers/.test(clientText))
        tags.add("performance");
    if (/b2b|corporativo|empresa|empresarial|indústria|industria/.test(clientText))
        tags.add("b2b");
    if (/local|loja|restaurante|clinica|clínica|servi[çc]o local|presencial|bairro/.test(clientText))
        tags.add("local_business");
    if (/e-?commerce|ecommerce|varejo|loja online|checkout|carrinho/.test(clientText))
        tags.add("ecommerce");
    if (/creator|criador|influencer|influenciador|youtube|instagram|tiktok|podcast/.test(clientText))
        tags.add("creator_business");
    return tags;
};
const normalizeFunnelStage = (value) => {
    const normalized = normalizeSearchText(String(value ?? ""));
    if (!normalized)
        return null;
    if (/^awareness$|^topo$|descoberta|alcance|descobrir|reconhecimento/.test(normalized))
        return "awareness";
    if (/^consideration$|consideracao|consideração|avaliacao|avaliação|pesquisa|comparacao|comparação/.test(normalized))
        return "consideration";
    if (/^decision$|decisao|decisão|fechamento|compra|conversao|conversão/.test(normalized))
        return "decision";
    if (/^retention$|retencao|retenção|recorrencia|recorrência|lifetime/.test(normalized))
        return "retention";
    if (/^scale$|escala|expansao|expansão|sistemat|operacao|operação/.test(normalized))
        return "scale";
    return null;
};
const buildFunnelStageSignals = (input) => {
    const stage = normalizeFunnelStage(input.funnelStage);
    const signals = new Set();
    if (stage === "awareness")
        signals.add("stage awareness");
    if (stage === "consideration")
        signals.add("stage consideration");
    if (stage === "decision")
        signals.add("stage decision");
    if (stage === "retention")
        signals.add("stage retention");
    if (stage === "scale")
        signals.add("stage scale");
    return { stage, signals };
};
const buildObjectiveSignals = (text) => {
    const normalized = normalizeSearchText(text);
    const signals = new Set();
    if (/autoridade|credibilidade|posicionamento|prova|case/.test(normalized))
        signals.add("authority");
    if (/educa|ensina|explica|clareza|mecanismo|awareness|alcance|topo/.test(normalized))
        signals.add("education");
    if (/convers|lead|venda|cta|agend|demo|checkout/.test(normalized))
        signals.add("conversion");
    if (/prova|resultado|depoimento|evidencia|case/.test(normalized))
        signals.add("proof");
    if (/obje|medo|risco|seguran|duvid/.test(normalized))
        signals.add("objection");
    if (/comunidade|engaj|recorr|retenc/.test(normalized))
        signals.add("community");
    if (/escala|processo|autom|eficien/.test(normalized))
        signals.add("scale");
    if (/viral|trend|tendenc|alcance/.test(normalized))
        signals.add("trend");
    return signals;
};
const scoreOrganicStrategyOption = (item, input) => {
    ...
    const catalogText = normalizeSearchText(collectText([item.strategyName, item.strategySummary, ...item.objectiveTags, ...item.productFitTags, ...item.fitSignals, ...item.recommendedFormats]));
    ...
    if (productFitTags.has("consultive") && item.id === "conversion_value")
        score += 6;
    if (productFitTags.has("consultive") && item.id === "authority_proof")
        score += 6;
    if (productFitTags.has("educational") && item.id === "education_mechanism")
        score += 8;
    if (productFitTags.has("educational") && item.id === "proof_reduction")
        score += 5;
    if (productFitTags.has("productized") && item.id === "scale_system")
        score += 8;
    if (productFitTags.has("productized") && item.id === "conversion_value")
        score += 4;
    if (productFitTags.has("high_ticket") && item.id === "authority_proof")
        score += 8;
    if (productFitTags.has("high_ticket") && item.id === "objection_breaker")
        score += 6;
    if (productFitTags.has("performance") && item.id === "conversion_value")
        score += 8;
    if (productFitTags.has("performance") && item.id === "scale_system")
        score += 5;
    if (productFitTags.has("creator_business") && item.id === "trend_amplification")
        score += 8;
    if (productFitTags.has("local_business") && item.id === "proof_reduction")
        score += 6;
    if (productFitTags.has("local_business") && item.id === "conversion_value")
        score += 5;
    ...
    const fitBand = fitScore >= 75 ? "recommended" : fitScore >= 60 ? "secondary" : "not_recommended";
    return {
        id: item.id,
...
export const buildContextualStrategyOptions = (input) => {
    const scoredOptions = ORGANIC_STRATEGY_CATALOG.map((item) => scoreOrganicStrategyOption(item, input)).sort((left, right) => right.fitScore - left.fitScore || right.confidence - left.confidence || left.strategyName.localeCompare(right.strategyName));
...
const inferCampaignActionFocus = (primary, context, product) => {
    ...
    if (/autoridade|posicionamento|credibil|case|prova/.test(text))
        return "autoridade e prova";
    if (/educa|ensina|explica|clareza|mecanismo|aula|didat/.test(text))
        return "educação e mecanismo";
    if (/convers|cta|lead|venda|agend|demo|checkout/.test(text))
        return "conversão e CTA";
    if (/obje|medo|risco|seguran|duvid/.test(text))
        return "quebra de objeções";
    if (/viral|trend|tendenc|alcance|distribu/.test(text))
        return "alcance e distribuição";
    if (/escala|processo|autom|efici/.test(text))
        return "escala e sistema";
    if (/comunidade|engaj|recorr|reten/.test(text))
        return "comunidade e retenção";
    const offerProfile = inferOfferProfile(product);
    if (offerProfile.isConsultative || offerProfile.isHighTicket)
        return "autoridade e prova";
    if (offerProfile.isEducational)
        return "educação e mecanismo";
    if (offerProfile.isProductized)
...
export const buildCampaignActionPlan = (primary, context, product) => {
    const stage = (normalizeFunnelStage(context.funnelStage) ?? normalizeFunnelStage(primary?.funnelStage) ?? "consideration");
...
const buildStrategyName = (kind, draft, product) => STRATEGY_THEME_LABELS[inferStrategyTheme(kind, {
...
const chooseActiveProduct = async (clientId, productId) => {
    if (productId) {
...
const normalizeStrategyLibraryKey = (strategyName, objective, funnelStage) => [strategyName, objective, funnelStage]
    ...
    .filter(Boolean)
    .join("__");
const scoreStrategyLibraryForContext = (strategy, product, context) => {
    const fitSignals = Array.isArray(strategy.fit_signals)
        ? strategy.fit_signals.map((signal) => (typeof signal === "string" ? signal : "")).filter(Boolean)
        : [];
    ...
    if (product?.status === "prioritized" || product?.status === "in_campaign")
        score += 8;
    if (String(strategy.fit_band ?? "").toLowerCase() === "recommended")
        score += 8;
    if (String(strategy.fit_band ?? "").toLowerCase() === "secondary")
        score += 4;
    if (offerProfile.isConsultative && strategyAngles.has("authority")) {
        score += 16;
    }
    ...
    if (fitSignals.some((signal) => signal.includes("produto prioritário")))
        score += 4;
    if (fitSignals.some((signal) => signal.includes("produto ativo")))
        score += 3;
    if (fitSignals.some((signal) => signal.includes("bom encaixe")))
        score += 5;
    if (fitSignals.some((signal) => signal.includes("compatível")))
        score += 4;
    ...
    if (stage) {
        if (stage === "awareness" && String(strategy.canonical_funnel_stage ?? "").toLowerCase() === "awareness")
            score += 10;
        if (stage === "consideration" && String(strategy.canonical_funnel_stage ?? "").toLowerCase() === "consideration")
            score += 10;
        if (stage === "decision" && String(strategy.canonical_funnel_stage ?? "").toLowerCase() === "conversion")
            score += 10;
        if (stage === "retention" && String(strategy.canonical_funnel_stage ?? "").toLowerCase() === "retention")
            score += 10;
        if (stage === "scale" && String(strategy.canonical_funnel_stage ?? "").toLowerCase() === "scale")
            score += 10;
    }
    return Math.max(0, Math.min(100, Math.round(score)));
};
const toRecommendationItemFromLibrary = (strategy, product, context) => {
    const fitSignals = Array.isArray(strategy.fit_signals)
        ? strategy.fit_signals.map((signal) => (typeof signal === "string" ? signal : "")).filter(Boolean)
        : [];
...
const scoreAssetForContext = (asset, product, context) => {
    const payload = asset.payload_json;
    const strategy = (payload.strategy ?? {});
    const recommendation = (payload.recommendation ?? {});
    const source = (payload.source ?? {});
    const primaryVideo = (source.primaryVideo ?? null);
    ...
    if (asset.kind === "primary")
        score += 12;
    if (asset.kind === "alternative")
        score += 6;
    if (asset.kind === "plan_b")
        score += 2;
    if (product?.is_active)
        score += 5;
    if (product?.status === "prioritized" || product?.status === "in_campaign")
    ...
    const stage = normalizeFunnelStage(context.funnelStage);
    if (stage) {
        if (stage === "awareness" && String(strategy.funnelStage ?? "").toLowerCase() === "awareness")
            score += 10;
        if (stage === "consideration" && String(strategy.funnelStage ?? "").toLowerCase() === "consideration")
            score += 10;
        if (stage === "decision" && String(strategy.funnelStage ?? "").toLowerCase() === "conversion")
            score += 10;
        if (stage === "retention" && String(strategy.funnelStage ?? "").toLowerCase() === "retention")
            score += 10;
        if (stage === "scale" && String(strategy.funnelStage ?? "").toLowerCase() === "scale")
            score += 10;
    }
    return Math.max(0, Math.min(100, Math.round(score)));
};
const toRecommendationItem = (asset, product, context) => {
    const payload = asset.payload_json;
    const strategy = (payload.strategy ?? {});
    const recommendation = (payload.recommendation ?? {});
    const fitSignals = Array.isArray(recommendation.fitSignals)
    ...
    return {
        id: asset.id,
        strategyKey: normalizeStrategyLibraryKey(String(strategy.name ?? buildFallbackStrategyName(asset.kind)), String(strategy.objective ?? null), String(strategy.funnelStage ?? null)),
        kind: asset.kind,
        version: asset.version,
        strategyName: String(strategy.name ?? buildFallbackStrategyName(asset.kind)),
        ...
        sourceUrls: toList((recommendation.sourceUrls ?? [])),
        evidence: toList((recommendation.evidence ?? [])),
        fitBand,
        sourceCount: 1,
    };
};
const sortRecommendations = (items) => [...items].sort((left, right) => right.fitScore - left.fitScore || right.confidence - left.confidence || left.version - right.version);
const buildStrategyEvidencePayload = (assets) => assets.map((asset) => {
    const payload = asset.payload_json;
    const strategy = (payload.strategy ?? {});
    const recommendation = (payload.recommendation ?? {});
    const source = (payload.source ?? {});
    ...
    const fitSignals = Array.isArray(recommendation.fitSignals)
        ? recommendation.fitSignals.map((signal) => (typeof signal === "string" ? signal : "")).filter(Boolean)
        : [];
    const strategyKey = normalizeStrategyLibraryKey(String(strategy.name ?? buildFallbackStrategyName(asset.kind)), String(strategy.objective ?? null), String(strategy.funnelStage ?? null));
    return {
        id: asset.id,
...
const buildSourceMetadataFromAssets = (assets) => {
    const source = assets[0]?.payload_json?.source;
    return buildStrategySourceMetadata(source);
};
const buildStrategyRecordTitleFromAssets = (assets, source) => {
    const payload = assets[0]?.payload_json;
    const title = toNullableString(payload?.title ?? source?.primaryVideo?.title);
    return title;
};
export const buildRecommendationFromLibrary = (strategies, context, product) => {
    const scored = sortRecommendations(strategies.map((strategy) => toRecommendationItemFromLibrary(strategy, product, {
...
export const persistStrategyIntelligenceFromYoutubeAnalysis = async (input) => {
    ...
    const recommendation = buildRecommendationFromLibrary(libraryStrategies, {
...
export const recommendStrategyForClient = async (input) => {
    ...
    const recommendation = buildRecommendationFromLibrary(strategyLibrary, {
...
export const buildRecommendationFromAssets = (assets, context, product) => {
...
export const getStrategyIntelligenceByAssetId = async (input) => {
    ...
    const source = buildStrategySourceMetadata(asset.payload_json?.source);
    const title = toNullableString(asset.payload_json?.title ?? source?.primaryVideo?.title);
    return {
        client,
        asset,
        title,
