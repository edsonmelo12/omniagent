...
import { findRenderedAssetById, getRenderedAssetFileInfo, hasRenderedAssetFile, hasRenderedAssetSoundtrack, hasRenderedAssetSubtitle, regenerateRenderedAssetForClient, refreshRenderedAssetsForClient, } from "./rendered-assets.service.js";
...
export const registerContentRoutes = (app) => {
    const setAttachmentDownloadHeaders = (reply, filePath) => {
    ...
    app.post("/api/v1/clients/:clientId/rendered-assets/:renderedAssetId/regenerate", async (request) => {
        ...
        const authContext = requireAuthContext(request);
        const agencyId = requireAgencyHeader(request);
        const { clientId, renderedAssetId } = request.params;
