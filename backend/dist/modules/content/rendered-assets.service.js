...
const ensureSchema = async () => {
    ...
    if (!schemaReady) {
        schemaReady = (async () => {
           ...
           content_production_package_id uuid null references content_production_packages(id) on delete set null,
           content_production_package_version integer null,
           ...
           unique (client_id, version)
            ...
            await query(`create index if not exists ${TABLE_NAME}_client_package_idx on ${TABLE_NAME} (client_id, content_production_package_version desc, schedule_version desc)`);
        })().catch((error) => {
            schemaReady = null;
            throw error;
        });
...
const safeText = (value, fallback = "") => (typeof value === "string" && value.trim().length > 0 ? value.trim() : fallback);
const hashText = (value) => Array.from(value).reduce((hash, char) => {
    const next = (hash << 5) - hash + char.charCodeAt(0);
    return next | 0;
}, 0);
const pickBySeed = (seed, options) => options[Math.abs(hashText(seed)) % options.length];
...
const getRenderPalette = (manifest) => RENDER_PALETTES[Math.abs(hashText(`${manifest.title}|${manifest.clientId}|${manifest.workflow.owner}`)) % RENDER_PALETTES.length];
...
const getArtDirectionForItem = (item, index) => {
...
const getAlternativeArtDirectionForItem = (item, index, currentKey) => {
...
const getItemPalette = (item, index) => {
...
const createPreviewDataUrl = (manifest) => {
...
const normalizeRenderedAssetManifest = (payload) => {
...
const getStyleConfig = (styleName = "") => {
...
const buildRenderedAssetHtml = (input) => {
    const selectedStyle = input.manifest.variantType === "headline" ? input.manifest.artDirection.palette : getRenderPalette(input.manifest);
    const title = safeText(input.manifest.variantType === "headline" && input.manifest.focusItem ? input.manifest.focusItem.title : input.contentPackagePayload?.visualDirection?.title, `${input.clientName} visual direction`);
    const tagline = safeText(input.manifest.variantType === "headline" && input.manifest.focusItem
        ? input.manifest.focusItem.objective
        : input.contentPackagePayload?.visualDirection?.firstPass, "direção visual profissional");
    const items = (input.manifest.variantType === "headline" ? [input.manifest.focusItem].filter(Boolean) : input.schedulePayload?.items ?? []).slice(0, 6);
    ...
    const shellBackground = input.manifest.variantType === "headline"
    ...
    const itemCards = input.manifest.variantType === "headline"
        ? `
...
const tryRenderScreenshot = async (input) => {
    const clientDir = join(OUTPUT_DIR, input.clientId);
    await mkdir(clientDir, { recursive: true });
...
const buildManifest = (input) => {
    ...
    const visualDirectionRules = Array.isArray(visualDirection?.rules)
        ? visualDirection.rules.filter((rule) => typeof rule === "string" && rule.trim().length > 0)
        : [];
    const visualDirectionCreativeSignals = Array.isArray(visualDirection?.creativeSignals) ? visualDirection.creativeSignals : [];
    const items = (schedulePayload?.items ?? []).filter((item) => Boolean(item));
    ...
    const baseManifest = {
        ...
        summary: visualDirection?.productImagePolicy ??
        ...
        items: items.map((item, index) => ({
            position: typeof item.position === "number" ? Number(item.position) : index + 1,
            title: safeText(item.title, `Item ${index + 1}`),
            channel: safeText(item.channel, "instagram"),
            format: safeText(item.format, "post"),
            pillar: safeText(item.pillar, "editorial"),
            angle: safeText(item.angle, "positioning"),
            status: safeText(item.status, "planned"),
            objective: safeText(item.objective, "Objetivo editorial"),
            reason: safeText(item.reason, "Razão editorial"),
            hook: safeText(item.hook, "Gancho editorial"),
            proof: safeText(item.proof, "Prova curta"),
            cta: safeText(item.cta, "Próximo passo"),
            visualCue: safeText(item.visualCue, "Layout editorial"),
        })),
        creativeSignals: (visualDirectionCreativeSignals.length > 0 ? visualDirectionCreativeSignals : items)
            .slice(0, 4)
            .map((signal, index) => ({
        ...
        references: Array.isArray(visualDirection?.references)
            ? visualDirection.references.filter((reference) => typeof reference === "string" && reference.trim().length > 0)
            : [],
...
const buildHeadlineManifest = (input) => {
    ...
    const variantLabel = `${channel} · ${format} · ${artDirection.label} (${input.itemIndex + 1}/${input.itemCount})${input.regeneration ? " · revisão" : ""}`;
    const manifest = {
        ...input.baseManifest,
        ...
        creativeSignals: [input.focusItem],
        references: [input.focusItem.title, input.focusItem.objective, input.focusItem.reason, input.focusItem.cta].filter((value) => typeof value === "string" && value.trim().length > 0),
        previewMimeType: "image/svg+xml",
        previewDataUrl: "",
        createdAt: new Date().toISOString(),
    };
...
const buildQueuePayloadFromManifest = (manifest, htmlContent) => ({
    manifest,
    ...manifest,
    status: "queued",
...
const createQueuedRenderedAssetFromManifest = async (input) => createRenderedAssetVersion({
    clientId: input.clientId,
...
const buildRemotionCopyMotionProps = (manifest, htmlContent) => {
...
const renderedAssetSelectColumns = `id, client_id, version, status, content_production_package_id, content_production_package_version,
...
const claimNextQueuedRenderedAsset = async () => {
    await ensureSchema();
    return withTransaction(async (client) => {
        const result = await client.query(`select ${renderedAssetSelectColumns}
       from ${TABLE_NAME}
       where status = 'queued'
       ...
       limit 1`);
        ...
        const claimed = await client.query(`update ${TABLE_NAME}
       set status = 'rendering',
           updated_at = now()
       where id = $1
       returning ${renderedAssetSelectColumns}`, [nextAsset.id]);
        return claimed.rows[0] ?? null;
    });
};
const getQueuePayload = (asset) => {
...
const renderQueuedRenderedAsset = async (asset) => {
...
const failRenderedAsset = async (asset, error) => {
    const queuePayload = getQueuePayload(asset);
    const payload = queuePayload?.manifest ?? asRecord(asset.payload_json);
...
export const startRenderedAssetQueueWorker = (options = {}) => {
    if (queueWorkerTimer) {
        return;
    }
...
export const findLatestRenderedAssetByClientId = async (clientId) => {
    await ensureSchema();
    return queryOne(`select id, client_id, version, status, content_production_package_id, content_production_package_version,
            schedule_id, schedule_version, render_engine, asset_format, asset_path,
            asset_mime_type, asset_width, asset_height, preview_data_url, preview_mime_type,
            html_content, payload_json, created_at, updated_at
     from ${TABLE_NAME}
...
export const listRenderedAssetsByClientId = async (clientId) => {
    await ensureSchema();
    return query(`select id, client_id, version, status, content_production_package_id, content_production_package_version,
            schedule_id, schedule_version, render_engine, asset_format, asset_path,
            asset_mime_type, asset_width, asset_height, preview_data_url, preview_mime_type,
            html_content, payload_json, created_at, updated_at
     from ${TABLE_NAME}
...
export const findRenderedAssetById = async (assetId) => {
    await ensureSchema();
    return queryOne(`select id, client_id, version, status, content_production_package_id, content_production_package_version,
            schedule_id, schedule_version, render_engine, asset_format, asset_path,
            asset_mime_type, asset_width, asset_height, preview_data_url, preview_mime_type,
...
export const createRenderedAssetVersion = async (input) => {
    ...
    return withTransaction(async (client) => {
        ...
        const result = await client.query(`insert into ${TABLE_NAME} (
         ...
         version,
         status,
         content_production_package_id,
         content_production_package_version,
         schedule_id,
         schedule_version,
         render_engine,
         asset_format,
       ...
       returning id, client_id, version, status, content_production_package_id, content_production_package_version,
                 schedule_id, schedule_version, render_engine, asset_format, asset_path,
                 asset_mime_type, asset_width, asset_height, preview_data_url, preview_mime_type,
                 html_content, payload_json, created_at, updated_at`, [
            input.clientId,
...
export const refreshRenderedAssetsForClient = async (input) => {
    ...
    const itemManifests = manifest.items.slice(0, 4).map((item, index) => buildHeadlineManifest({
...
export const regenerateRenderedAssetForClient = async (input) => {
    ...
    const normalizedMode = input.mode;
    ...
    const alternateDirection = sourceIsHeadline && sourceFocusItem
        ? getAlternativeArtDirectionForItem(sourceFocusItem, sourceFocusItem.position - 1, sourceArtDirection?.key ?? null)
        : sourceArtDirection;
    const regeneratedHeadlineManifest = sourceIsHeadline && sourceFocusItem
    ...
    const regeneratedAsset = sourceIsHeadline && regeneratedHeadlineManifest
        ? await createQueuedRenderedAssetFromManifest({
            clientId: input.clientId,
            version: baseVersion,
            contentPackageId: sourceAsset.content_production_package_id ?? contentPackage.id,
            contentPackageVersion: sourceAsset.content_production_package_version ?? contentPackage.version,
...
export const getRenderedAssetFileInfo = (asset) => {
    const payload = asRecord(asset.payload_json);
    return {
...
export const hasRenderedAssetSoundtrack = (asset) => {
    const payload = asRecord(asset.payload_json);
