...
import { createContentPackageVersion, createContentPlanVersion, createScheduleVersion, findLatestContentPackageByClientId, findLatestContentPlanByClientId, listContentPlansByClientId, listContentPackagesByClientId, listSchedulesByClientId, } from "./content.repository.js";
import { findLatestRenderedAssetByClientId, listRenderedAssetsByClientId, refreshRenderedAssetsForClient, } from "./rendered-assets.service.js";
import { getOfferProfileSignals } from "../offer-profile/offer-profile.service.js";
const getResearchPayload = (marketResearch) => {
    if (!marketResearch || typeof marketResearch.payload_json !== "object" || marketResearch.payload_json === null) {
        return null;
    }
    return marketResearch.payload_json;
};
const asRecord = (value) => (typeof value === "object" && value !== null ? value : null);
const asString = (value) => (typeof value === "string" && value.trim().length > 0 ? value.trim() : null);
const asArray = (value) => (Array.isArray(value) ? value : []);
const asStringArray = (value) => asArray(value).map((item) => asString(item)).filter((item) => Boolean(item));
...
const getBrandProfileSignals = (brandProfile) => {
    if (!brandProfile) {
        return null;
    }
...
const compactPhrase = (value, maxWords) => value.trim().split(/\s+/g).filter(Boolean).slice(0, maxWords).join(" ");
...
const buildPautaCreativeSignal = (input) => {
    const focus = compactPhrase(firstText(input.offerProfile?.promise, input.offerProfile?.audience, input.brandProfile?.uniqueValueProposition, input.brandProfile?.promise, input.differentiator, input.marketSummary, input.objective, input.clientName), 7);
    const seed = `${input.clientName}|${input.pillar}|${input.angle}|${input.objective}|${input.reason}|${focus}|${input.index}`.toLowerCase();
    const needsProofBackstop = input.brandProfile?.proofGaps.includes("testimonials") ||
...
const getEditorialPautas = (contentPlan) => {
    ...
    const payload = contentPlan.payload_json;
...
const getPostArtGalleryPayload = (gallery) => {
    if (!gallery || typeof gallery.payload_json !== "object" || gallery.payload_json === null) {
        return null;
    }
    return gallery.payload_json;
};
const getPostArtGalleryContext = async (clientId) => {
...
const getPostArtGalleryVariantCount = (row) => {
...
const buildPostArtGalleryComparison = (rows) => {
...
export const buildEditorialPautas = (input) => {
    ...
    const stageLabel = input.marketStage === "scale"
        ? "escala"
    ...
    return pillars.slice(0, 4).map((pillar, index) => {
        ...
        const differentiator = input.offerProfile?.promise ??
        ...
        const objective = input.offerProfile?.proofGaps.includes("testimonials") ||
            input.offerProfile?.proofGaps.includes("reviews") ||
...
export const buildVisualDirectionPayload = (input) => ({
    ...
    productImagePolicy: input.offerProfile && input.brandProfile
...
const getClientOfferContext = async (clientId) => {
...
export const getContentContext = async (input) => {
...
export const createContentPlan = async (input) => {
    ...
    const contentPackage = await createContentPackageVersion({
        ...
        payload: {
            ...
            items: editorialPautas.length > 0
...
export const reviseContentPlan = async (input) => {
    await requireAgencyAccess(input.userId, input.agencyId);
    const client = await findClientById(input.agencyId, input.clientId);
    if (!client) {
        throw createError("NOT_FOUND", "Client not found", 404);
    ...
    const existingPayload = typeof existingPlan.payload_json === "object" && existingPlan.payload_json !== null ? existingPlan.payload_json : {};
    const researchPayload = getResearchPayload(marketResearch);
    const editorialPillars = researchPayload?.recommendations?.editorialPillars ?? [];
    const differentiators = researchPayload?.recommendations?.differentiators ?? [];
    const contentAngles = researchPayload?.recommendations?.contentAngles ?? [];
    ...
    const contentPlanPayload = {
        ...
        clientRecordVersion: clientRecord.version,
        proposalVersion: proposal.version,
        marketResearchVersion: marketResearch?.version ?? existingPayload.marketResearchVersion ?? null,
        marketStage: researchPayload?.market?.stage ?? existingPayload.marketStage ?? null,
        marketSummary: researchPayload?.market?.summary ?? existingPayload.marketSummary ?? null,
        brandProfileVersion: brandProfileSignals?.version ?? existingPayload.brandProfileVersion ?? null,
        brandProfileStatus: brandProfileSignals?.status ?? existingPayload.brandProfileStatus ?? null,
        brandProfileConfidence: brandProfileSignals?.confidence ?? existingPayload.brandProfileConfidence ?? null,
        brandSummary: brandProfileSignals?.summary ?? existingPayload.brandSummary ?? null,
        brandTone: brandProfileSignals?.toneOfVoice ?? existingPayload.brandTone ?? null,
        brandVisualPalette: brandProfileSignals?.visualPalette ?? existingPayload.brandVisualPalette ?? [],
        brandPrimaryCta: brandProfileSignals?.primaryCta ?? existingPayload.brandPrimaryCta ?? null,
        brandProofGaps: brandProfileSignals?.proofGaps ?? existingPayload.brandProofGaps ?? [],
        offerProfileVersion: offerProfile?.version ?? existingPayload.offerProfileVersion ?? null,
        offerProfileStatus: offerProfile?.status ?? existingPayload.offerProfileStatus ?? null,
        offerProfileConfidence: offerProfile?.confidence ?? existingPayload.offerProfileConfidence ?? null,
        offerSummary: offerProfile?.summary ?? existingPayload.offerSummary ?? null,
        offerAudience: offerProfile?.audience ?? existingPayload.offerAudience ?? null,
        offerPromise: offerProfile?.promise ?? existingPayload.offerPromise ?? null,
        offerProblemSolved: offerProfile?.problemSolved ?? existingPayload.offerProblemSolved ?? null,
        offerPrimaryCta: offerProfile?.primaryCta ?? existingPayload.offerPrimaryCta ?? null,
        offerTone: offerProfile?.toneOfVoice ?? existingPayload.offerTone ?? null,
        offerProofPoints: offerProfile?.proofPoints ?? existingPayload.offerProofPoints ?? [],
        offerProofGaps: offerProfile?.proofGaps ?? existingPayload.offerProofGaps ?? [],
        contentRhythm,
        objective: input.objective ?? existingPayload.objective ?? null,
        createdFrom: "manual-edit",
        revisedFromContentPlanVersion: existingPlan.version,
        note: input.note ?? null,
        pillars: editorialPillars.length > 0 ? editorialPillars : existingPayload.pillars ?? ["proof", "educational", "commercial", "community"],
        differentiators: differentiators.length > 0 ? differentiators : existingPayload.differentiators ?? [],
        contentAngles: contentAngles.length > 0 ? contentAngles : existingPayload.contentAngles ?? [],
        editorialPautas,
        scoreContext: {
            presence: snapshot.presence_score,
            consistency: snapshot.consistency_score,
...
export const createSchedule = async (input) => {
    ...
    const editorialPautas = getEditorialPautas(contentPlan);
    const contentRhythm = typeof contentPlanPayload?.contentRhythm === "string"
...
export const reviseSchedule = async (input) => {
    await requireAgencyAccess(input.userId, input.agencyId);
    const client = await findClientById(input.agencyId, input.clientId);
    if (!client) {
        throw createError("NOT_FOUND", "Client not found", 404);
    ...
    const payload = latestSchedule.payload_json;
    const nextPayload = {
        ...payload,
        title: typeof payload.title === "string" ? payload.title : `${client.name} schedule`,
        contentPlanVersion: contentPlan.version,
        startDate: input.startDate ?? payload.startDate ?? new Date().toISOString().slice(0, 10),
        endDate: input.endDate ?? payload.endDate ?? new Date(Date.now() + 1000 * 60 * 60 * 24 * 28).toISOString().slice(0, 10),
