...
const scorePresence = (profilesCount) => {
    if (profilesCount >= 4)
        return 90;
    if (profilesCount === 3)
        return 80;
...
const scoreConsistency = (profilesCount) => {
    if (profilesCount >= 4)
        return 85;
    if (profilesCount === 3)
        return 75;
...
const scoreProof = (profilesCount) => {
    if (profilesCount >= 3)
        return 70;
    if (profilesCount === 2)
        return 55;
...
const scoreConversionReadiness = (profilesCount) => {
    if (profilesCount >= 4)
        return 80;
    if (profilesCount === 3)
        return 70;
...
const buildOpportunityNotes = (profilesCount) => {
    if (profilesCount === 0) {
...
const dedupeSnapshotSources = (sources) => {
    const seen = new Set();
    const unique = [];
...
export const buildSocialIntelligenceSnapshot = async (input) => {
    ...
    const sourceRows = await createSnapshotSources(snapshot.id, dedupeSnapshotSources([
...
export const getLatestSocialIntelligence = async (input) => {
    ...
    const profiles = await listSocialProfilesByClientId(input.clientId);
    const sources = await queryOne(`select coalesce(json_agg(json_build_object(
     ...
     where sis.snapshot_id = $1`, [snapshot.id]);
    return {
        client,
        profiles,
        snapshot,
