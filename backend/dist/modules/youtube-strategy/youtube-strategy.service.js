...
import { createYoutubeStrategyAnalysis, ensureYoutubeStrategyVideoStateBackfill, findLatestYoutubeStrategyAnalysisByClientId, listYoutubeStrategyAnalysesByClientId, listYoutubeStrategyVideoStatesByClientId, updateYoutubeStrategyAnalysisPayload, upsertYoutubeStrategyVideoState, } from "./youtube-strategy.repository.js";
...
let transcriptPythonBootstrapPromise = null;
...
const validateTranscriptPython = async (pythonBinary) => {
    await execFileAsync(pythonBinary, ["-c", transcriptPythonValidationScript], {
        maxBuffer: 1024 * 1024,
    });
};
const installTranscriptPythonPackages = async (pythonBinary) => {
...
const labelForTranscriptLanguage = (languageCode) => {
    if (!languageCode)
        return null;
...
const normalizeTranscriptFingerprintText = (value) => compactText(value).toLowerCase();
export const buildTranscriptFingerprint = (transcript) => {
...
export const dedupeYoutubeVideoUrls = (videoUrls) => {
    ...
    for (const videoUrl of videoUrls) {
        ...
        if (videoId) {
            ...
            if (seenFallbackUrls.has(trimmed))
                continue;
...
const buildCanonicalVideoUrl = (videoId) => `https://www.youtube.com/watch?v=${videoId}`;
const extractJsonArrayFromHtml = (html, marker) => {
...
const scoreTrack = (languageCode, kind) => {
...
const parseTranscriptSegments = (body) => {
    ...
    for (const match of trimmed.matchAll(regex)) {
        ...
        const text = compactText(decodeHtmlEntities(rawText
            .replace(/<[^>]+>/g, "")
            .replace(/\n+/g, " ")
            .trim()));
        if (!text)
...
const splitIntoSentences = (text) => text
    ...
    .filter(Boolean);
const extractMatchingSentences = (sentences, keywords, limit = 3) => {
...
const buildStrategyInsights = (sentences) => STRATEGY_DEFINITIONS.map((definition) => {
...
const buildMechanisms = (strategies, sentences) => {
...
const buildTactics = (sentences) => {
    const tactics = [];
    const addIfMatches = (keywords, tactic) => {
...
const buildRisks = (analysisCount, transcriptCoverage, sentenceCount) => {
    const risks = new Set();
    if (analysisCount > 1) {
...
const buildBusinessApplicability = (text, strategies) => {
    ...
    const result = Object.entries(BUSINESS_TYPE_LABELS).map(([businessType, label]) => {
        const keywords = BUSINESS_TYPE_KEYWORDS[businessType];
        let score = 45;
        const matched = keywords.filter((keyword) => lowered.includes(keyword)).length;
        score += matched * 10;
        if (businessType === "local_business" && (categories.has("conversion") || categories.has("proof")))
            score += 15;
        if (businessType === "service_business" && (categories.has("positioning") || categories.has("conversion")))
            score += 12;
        if (businessType === "physical_product" && (categories.has("offer") || categories.has("distribution")))
            score += 12;
        if (businessType === "ecommerce" && (categories.has("distribution") || categories.has("conversion")))
            score += 12;
        if (businessType === "saas" && (categories.has("operations") || categories.has("retention")))
            score += 12;
        if (businessType === "b2b" && (categories.has("proof") || categories.has("operations")))
            score += 12;
        if (businessType === "info_product" && (categories.has("positioning") || categories.has("proof")))
            score += 12;
        if (businessType === "creator_business" && categories.has("distribution"))
            score += 12;
...
const buildExecutiveSummary = (videos) => {
    if (videos.length === 0) {
...
const buildGlobalThesis = (videos) => {
...
const buildNextTests = (businessApplicability) => {
...
const chooseTranscriptLanguage = (languageCode, language) => languageCode ?? language ?? "unknown";
...
const fetchYoutubeHtml = async (videoId) => {
...
const fetchTranscriptSegments = async (trackBaseUrl) => {
...
const fetchTranscriptSegmentsViaPython = async (videoId, languages) => {
...
const fetchTranscriptSegmentsViaWhisper = async (videoUrl, modelName) => {
    const python = await ensureTranscriptPython();
    const script = [
        "import json",
        "import os",
...
const analyzeVideo = async (input) => {
    ...
    const evidence = strategyInsights.flatMap((item) => item.evidence.slice(0, 2).map((sentence) => ({
    ...
    const qualityScore = clampScore(Math.min(100, Math.round((transcript.characterCount / 45) * (transcript.segmentCount > 0 ? 1 : 0)) + (sentences.length >= 8 ? 15 : 0)));
    const transcriptFingerprint = buildTranscriptFingerprint({
        status: transcript.status,
        text: transcript.text,
    });
    const summary = strategyInsights.length > 0
    ...
    const thesis = strategyInsights[0]?.label ??
        (transcript.status === "available"
...
const buildPayload = async (input) => {
    ...
    const videoResults = await Promise.all(uniqueVideoUrls.map(async (videoUrl) => analyzeVideo({ videoUrl, notes: input.notes }).catch((error) => {
    ...
    const aggregateApplicability = Object.entries(videos.reduce((accumulator, video) => {
    ...
    const analysisStatus = totalAvailable > 0 ? (totalMissing > 0 ? "partial" : "complete") : "failed";
...
const loadYoutubeStrategyContext = async (input) => {
...
const buildLatestAnalysisResponse = (analysis) => analysis;
export const getYoutubeStrategyContext = async (input) => {
...
export const createYoutubeStrategyAnalysisForClient = async (input) => {
    ...
    let strategyIntelligencePayload = {
        ...buildResult.payload,
    ...
    if (shouldCreateNewAnalysis) {
        ...
        strategyIntelligencePayload = {
            ...
            strategyLibrary: {
                ...
                ...strategyIntelligence.payload.strategyLibrary,
                assets: (strategyIntelligence.payload.strategyLibrary?.assets ?? []).map((asset) => asset),
            },
            strategyIntelligence: {
                ...strategyIntelligence.payload.strategyIntelligence,
                assets: (strategyIntelligence.payload.strategyIntelligence?.assets ?? []).map((asset) => asset),
            },
        };
        latestAnalysis = await updateYoutubeStrategyAnalysisPayload(createdAnalysis.id, strategyIntelligencePayload);
        await Promise.all(buildResult.processing.videos.map((video) => upsertYoutubeStrategyVideoState({
            clientId: context.client.id,
