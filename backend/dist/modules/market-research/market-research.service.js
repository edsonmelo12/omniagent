...
import { createMarketResearchVersion, findLatestMarketResearchByClientId, listMarketResearchesByClientId, updateMarketResearchVersion, } from "./market-research.repository.js";
const clampScore = (value) => Math.max(0, Math.min(100, Math.round(value)));
const chooseMarketStage = (snapshot, profilesCount) => {
    if (!snapshot) {
        return "presence";
    }
...
const summarizeMarketStage = (stage) => {
    if (stage === "scale") {
...
const buildCompetitorSignal = async (input) => {
...
const appendNotes = (baseNotes, additions) => {
    const content = additions.filter((addition) => typeof addition === "string" && addition.trim().length > 0).join("\n");
    if (!baseNotes) {
...
const buildRecommendations = (stage) => {
    if (stage === "scale") {
        return {
...
const buildMarketResearchPayload = async (input) => {
    ...
    const sourceSignals = await Promise.all([...new Set([...input.sourceUrls, ...input.competitorUrls, ...input.publicSignals.sources.map((source) => source.sourceUrl)].filter(Boolean))].map(async (url) => buildCompetitorSignal({ url, role: input.competitorUrls.includes(url) ? "competitor" : "source" }).catch(() => ({
        url,
    ...
    const benchmarkSummary = competitiveDensity > 0
    ...
    return {
        ...
        competition: {
            ...
            benchmarkSummary,
            summary: input.competitorUrls.length > 0
...
const loadResearchContext = async (input) => {
...
export const createMarketResearch = async (input) => {
    ...
    const hasSignals = context.profiles.length > 0 ||
...
export const reviseMarketResearch = async (input) => {
    ...
    const previousPayload = latestMarketResearch.payload_json;
    ...
    const previousMarket = typeof previousPayload.market === "object" && previousPayload.market !== null ? previousPayload.market : null;
    const previousCompetition = typeof previousPayload.competition === "object" && previousPayload.competition !== null
        ? previousPayload.competition
        : {};
    const previousSourceUrls = Array.isArray(previousCompetition.sourceUrls) ? previousCompetition.sourceUrls : [];
    const previousCompetitorUrls = Array.isArray(previousCompetition.competitorUrls) ? previousCompetition.competitorUrls : [];
    ...
    const basePayload = await buildMarketResearchPayload({
        ...
        note: input.note ?? previousPayload.note,
        publicOnly: input.publicOnly,
        confidence: input.confidence ?? context.snapshot?.confidence ?? 75,
        publicSignals,
    });
