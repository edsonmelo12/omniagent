...
const getNextVersion = async (productId) => {
    await ensureOfferProfileSchema();
    const row = await queryOne(`select coalesce(max(version), 0) + 1 as version
     from ${TABLE_NAME}
     where client_product_id = $1`, [productId]);
    return Number(row?.version ?? 1);
};
export const listOfferProfilesByClientId = async (clientId) => (async () => {
    await ensureOfferProfileSchema();
    return query(`select
       id,
       client_id,
...
export const listOfferProfilesByProductId = async (productId) => (async () => {
    await ensureOfferProfileSchema();
    return query(`select
       id,
       client_id,
     ...
     order by version desc, created_at desc`, [productId]);
})();
export const findLatestOfferProfileByClientId = async (clientId) => (async () => {
    await ensureOfferProfileSchema();
...
export const findLatestOfferProfileByProductId = async (productId) => (async () => {
    await ensureOfferProfileSchema();
    return queryOne(`select
       id,
       client_id,
...
export const createOfferProfileVersion = async (input) => withTransaction(async (client) => {
    await ensureOfferProfileSchema();
