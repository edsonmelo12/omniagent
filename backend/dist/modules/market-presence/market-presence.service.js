...
import { findSocialIntelligenceSnapshotById, listSnapshotSourcesBySnapshotId, listLatestSnapshotByClientId, } from "../social-intelligence/social-intelligence.repository.js";
import { createMarketPresenceBaselineVersion, createMarketPresenceCheckpointVersion, createMarketPresenceComparison, createMarketPresenceIntervention, ensureMarketPresenceTables, findLatestMarketPresenceBaselineByClientId, findLatestMarketPresenceCheckpointByClientId, findLatestMarketPresenceComparisonByClientId, findLatestMarketPresenceInterventionByClientId, findMarketPresenceBaselineById, findMarketPresenceCheckpointById, findMarketPresenceInterventionById, listMarketPresenceBaselinesByClientId, listMarketPresenceCheckpointsByClientId, listMarketPresenceComparisonsByClientId, listMarketPresenceInterventionsByClientId, } from "./market-presence.repository.js";
import { resolveMarketPresenceScope } from "../market-research/market-geography.js";
const clampScore = (value) => Math.max(0, Math.min(100, Math.round(value)));
const calculateMaturity = (presence, consistency, proof, conversion) => clampScore(Math.round((presence + consistency + proof + conversion) / 4));
const inferScope = (marketContext, clientSegment) => resolveMarketPresenceScope({
...
const stateLabel = (maturity) => {
    if (maturity >= 80)
        return "presença madura";
    if (maturity >= 60)
        return "presença funcional";
    if (maturity >= 40)
        return "base em construção";
    return "presença inicial";
};
const buildAnalogy = (maturityDelta, reading) => {
...
const buildReading = (baseline, checkpoint) => {
    const maturityDelta = calculateMaturity(checkpoint.presence_score, checkpoint.consistency_score, checkpoint.proof_score, checkpoint.conversion_score) - calculateMaturity(baseline.presence_score, baseline.consistency_score, baseline.proof_score, baseline.conversion_score);
...
const summarizeScores = (presence, consistency, proof, conversion, confidence, profilesCount, sourceCount) => `P ${presence}/100, C ${consistency}/100, Pr ${proof}/100, Cv ${conversion}/100, confiança ${confidence}/100, perfis ${profilesCount}, fontes ${sourceCount}.`;
const buildBaselinePayload = (input) => ({
    ...
    summary: summarizeScores(input.presenceScore, input.consistencyScore, input.proofScore, input.conversionScore, input.confidenceScore, input.profilesCount, input.sourceCount),
...
const buildComparisonPayload = (input) => ({
    baseline: {
        ...
        summary: summarizeScores(input.baseline.presence_score, input.baseline.consistency_score, input.baseline.proof_score, input.baseline.conversion_score, input.baseline.confidence_score, input.baseline.profiles_count, input.baseline.source_count),
    },
    checkpoint: {
        id: input.checkpoint.id,
        version: input.checkpoint.version,
        collectedAt: input.checkpoint.collected_at,
        summary: summarizeScores(input.checkpoint.presence_score, input.checkpoint.consistency_score, input.checkpoint.proof_score, input.checkpoint.conversion_score, input.checkpoint.confidence_score, input.checkpoint.profiles_count, input.checkpoint.source_count),
    },
...
const collectEvidenceUrls = (args) => buildEvidenceUrls([
    args.clientWebsiteUrl,
    ...args.snapshotSources.map((source) => source.source_url),
    ...args.profileUrls,
    ...args.inputEvidenceUrls,
]);
const getPublicSignalSourceCount = (marketResearch) => {
    ...
    const sourceCount = publicSignals.sourceCount;
    return typeof sourceCount === "number" && Number.isFinite(sourceCount) ? sourceCount : 0;
};
const resolveSnapshotSources = async (snapshotId) => {
    if (!snapshotId) {
        return [];
    }
    return listSnapshotSourcesBySnapshotId(snapshotId);
};
const mapScope = (scope) => scope === "local" || scope === "regional" || scope === "nacional" || scope === "nicho" || scope === "hibrido" ? scope : "hibrido";
const resolveSourceSnapshot = async (snapshotId, clientId) => {
    if (snapshotId) {
...
export const getMarketPresenceContext = async (input) => {
    ...
    const [latestBaseline, latestIntervention, latestCheckpoint, latestComparison, baselines, interventions, checkpoints, comparisons, snapshot, clientRecord, marketResearch, profiles] = await Promise.all([
...
const buildComparison = async (clientId, baseline, checkpoint) => {
    ...
    const executiveSummary = readingInfo.reading === "insufficient_data"
