...
import { createSocialPresenceSnapshots, ensureSocialPresenceTables, listLatestSocialPresenceSnapshotsByClientId, listSocialPresenceSnapshotsByClientId, } from "./social-presence.repository.js";
...
const groupByPlatform = (snapshots) => {
...
export const buildSocialPresenceSummary = (snapshots) => {
    ...
    const networks = [...grouped.entries()]
        ...
        return {
            ...
            followersDelta: latest.followers_count !== null && previous?.followers_count !== null && previous?.followers_count !== undefined
                ? latest.followers_count - previous.followers_count
                : null,
            postsCount: latest.posts_count,
            postsDelta: latest.posts_count !== null && previous?.posts_count !== null && previous?.posts_count !== undefined
    ...
    const lastCapturedAt = snapshots.reduce((latest, row) => {
        if (!latest)
            return row.collected_at;
        return new Date(row.collected_at).getTime() > new Date(latest).getTime() ? row.collected_at : latest;
    }, null);
    const latestPostAt = snapshots.reduce((latest, row) => {
        if (!row.latest_post_at)
            return latest;
        if (!latest)
            return row.latest_post_at;
...
export const captureSocialPresence = async (input) => {
    ...
    const snapshots = await createSocialPresenceSnapshots(observations.map((observation) => ({
