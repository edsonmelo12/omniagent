...
3. **Resolve skills lazily**: Do not read every skill listed in `squad.yaml` at startup.
   Resolve only the skills required by the current step, route, or asset format.
   For each required non-native skill (anything other than web_search, web_fetch):
   ...
   Lazy resolution preserves quality while avoiding unnecessary skill context.
...
5. **Inject skill instructions selectively**: Check which skills the agent declares in its frontmatter `skills:`, then load only the skills required by the current route, step and asset format.
   Do not inject every declared visual format skill by default. For Visual Director and Creative Renderer, load base skills plus exactly one native format skill per asset.
   For each required non-native skill:
   ...
   c. Append to the agent's context, after format injection:
      ...
      ```
    d. Follow declaration order in the agent's frontmatter for required multi-skill injection

    **Mandatory social generation gate**: when the step creates, directs, renders, reviews, exports or delivers social assets, also read `squads/{name}/pipeline/data/skill-invocation-gate.md`. The step output must include a `Skill Invocation Ledger` proving every required skill was loaded and applied. Missing skill evidence is a veto, not a warning.

    **Mandatory incident trace gate**: when the step corrects, regenerates, relinks or re-approves an asset that was already reviewed/approved, or when the user reports a defect in a prior output, also read `squads/{name}/pipeline/data/pipeline-incident-trace-template.md`. The relevant reviewer/auditor output must either attach an `incident-trace-*` file or explicitly prove the template triggers do not apply. Missing incident trace evidence is a veto for corrected outputs.
...
- In the Task prompt, include:
  ...
  - The veto conditions from the step file (agent should self-check before completing)
   - The company context only when the task depends on agency-level context
   - `squads/{name}/state-summary.md` for quick status and routing context
   - The squad memory only for full diagnosis, strategic decisions, incident review, or when the step explicitly needs historical learning
   - The loaded skill instructions required by the current route and asset format, not every skill declared by the agent
   - The `skill-invocation-gate.md` rule when the step involves social asset generation
   - The `fast-safe-routing-policy.md` rule when choosing between quick, fast-track and full pipeline routes
  - The `pipeline-incident-trace-template.md` rule when the step is correcting, regenerating, relinking or re-approving a prior reviewed/approved output
...
- If the agent declares `skills:` and the step involves social asset generation, inline execution is allowed only after reading every required `skills/{skill}/SKILL.md` and `squads/{name}/pipeline/data/skill-invocation-gate.md`. The inline output must include the same `Skill Invocation Ledger` required from subagents. If the runner cannot guarantee this, stop and dispatch the declared subagent instead.
- If the inline step corrects, regenerates, relinks or re-approves a prior reviewed/approved output, inline execution is allowed only after reading `squads/{name}/pipeline/data/pipeline-incident-trace-template.md` and enforcing whether an Incident Trace is required. If the runner cannot guarantee this, stop and dispatch the declared subagent instead.
- If the inline step is a quick/status/routing task, prefer `state-summary.md` and avoid broad reads unless the user asks for details.
...
### Incident Trace Enforcement

When a step output is a correction/regeneration/relinking/re-approval of a prior
reviewed or approved artifact:

1. Read `squads/{name}/pipeline/data/pipeline-incident-trace-template.md`.
2. Check whether any trigger condition applies.
3. If a trigger applies, verify that an `incident-trace-*` file was produced in the client review folder.
4. If the trace is missing or lacks origin, why-it-passed, root cause and mitigation, treat this as a veto and loop the auditor/reviewer back for correction.
5. Do not proceed to user checkpoint for corrected outputs until the Incident Trace requirement is satisfied.

### Campaign Hub Auto-Regeneration

Whenever any step updates the `campaign-manifest.json` (or any file that affects asset links, preview paths, or asset status):

1. **MANDATORY**: Run `node squads/social-growth/scripts/regenerate-hub.mjs --client <client>` after the manifest write.
2. This regenerates `campaign-hub.html` from the manifest so all preview links are current.
3. **Never** edit `campaign-hub.html` manually — it is always generated from the manifest.
4. If the hub regeneration fails, treat this as a veto: the step is not complete until the hub reflects the manifest.

This rule applies to all agents: Creative Renderer, Reviewer, Pipeline Auditor, Content Repurposer, and any step that modifies the manifest or asset preview paths.
...
2. **Wait for animation (optional)** — Only wait if dashboard animation is explicitly enabled for the environment/session.
   Default behavior is to skip this wait to keep execution fast. If enabled, run via Bash tool:
   ```bash
   sleep 3
   ```
