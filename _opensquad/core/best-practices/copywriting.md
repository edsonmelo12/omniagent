...
10. **Public copy only.** The final piece must contain only what will be published. Internal labels, draft notes, direction words, and process commentary stay out of the visible copy. `Hook`, `CTA`, `briefing`, `autoridade de marca`, and `negocio premium` can exist in working notes, never as audience-facing text.
...
Do not expose the drafting scaffold in the public piece. If you are preparing a briefing or review artifact, keep the labels there; if you are preparing publishable copy, strip all labels and commentary.
