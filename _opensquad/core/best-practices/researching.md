...
9. **Commercial intelligence first** — When the research exists to feed a commercial proposal, the brief must also extract and label:
   - Target audience
   - Buyer personas
   - Unique value proposition
   - Brand archetype
   - Visual identity cues
   - Logo status / logo URL
   - Product imagery direction
   - Proof assets and objection handling
   These fields must be split into confirmed, inferred, and prescriptive signals.

10. **Semantic normalization first** — When a title, prompt, or brief contains ambiguous terms, resolve them into the most likely domain-specific meaning before searching or generating assets. Do not let the most literal keyword dominate the result when the client niche points elsewhere.

11. **Stage separation** — Research stays analytical. It may identify presence digital signals from the site, blog and social channels, market conditions, competitors, audience hypotheses, and trend signals, but it must not turn those signals into content calendars, published copy, or proposal decks unless that downstream artifact is explicitly requested.
...
When the downstream goal is a proposal, include a second block after the standard brief:

1. **Client Intelligence**
   - Company description
   - Presence digital map (site, blog, social channels, activity and proof)
   - Segment and buying context
   - Primary and secondary audience hypotheses
...
3. **Brand and Creative Intelligence**
   - Brand archetype
   - Tone of voice
   - Visual identity cues
   - Logo status and logo URL
   - Product imagery direction
   - Image risks and constraints

4. **Proposal Readiness**
   - Suggested commercial angle
   - Missing evidence
   - Confidence score for each major inference
   - Recommended next verification step if any field is uncertain

### Trend Signals for Later Content Work

Trend signals should be captured during research as market evidence only:

- recurring topics and language
- emerging objections or buyer questions
- format preferences visible in the market
- shifts in competitor positioning
- lifecycle notes such as emerging, growth, mature or declining

These signals inform strategy and future content planning. They do not become the content plan during research, and they do not generate a calendar until the content phase is explicitly triggered.

### Proposal Trigger Rule

Do not generate a commercial proposal report by default.

Generate it only when:

- the user explicitly requests a proposal or report, or
- the squad reaches a point where commercial packaging is required for the next decision.

When the proposal is not yet needed, keep the research output factual and reusable, and stop at analysis plus opportunity framing.

### Audience Rule

Treat audience and persona as hypotheses derived from evidence, not as fictional marketing personas.

Prefer labels that can be defended from public signals:
...
If the evidence is weak, state the gap instead of inventing persona detail.

### Central Client Intent Profile

When the downstream goal is not only research, but also image selection, copy drafting, or proposal generation, produce a single normalized interpretation block for the client and reuse it across the pipeline.

This profile should answer:
- What does the title actually mean in this niche?
- Which domain vocabulary is preferred?
- Which literal terms are ambiguous?
- Which terms should be prioritized?
- Which terms should be excluded?
- What fallback direction is safe if no exact match exists?
...
Use this block as the canonical semantic reference for:
- image search
- prompt generation
- copy briefing
- proposal brief creation

Follow the template in `client-intent-profile.md` when you need to persist or reuse
this interpretation across squads.

Rule of thumb:
- the title provides the raw input
- the client niche resolves the meaning
- the normalized intent drives downstream search
