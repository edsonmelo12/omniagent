...
If the squad is content-, visual-, or proposal-oriented, also compile a canonical `pipeline/data/client-intent-profile.md` from the research so downstream agents share the same semantic interpretation of the client, niche, and ambiguous terms.
...
- **Central Client Intent Profile** → `pipeline/data/client-intent-profile.md` (canonical interpretation, search rules, and fallback direction)
...
- Normalize ambiguous terms into a central client intent profile before searching; the resolved meaning must guide downstream image, copy, and proposal work
