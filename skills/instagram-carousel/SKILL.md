...
This skill is the Instagram-specific specialization of the shared
`social-visual-system`.

Before making format-specific decisions, read:

- `../social-visual-system/SKILL.md`
- `../social-visual-system/references/brand-intake.md`
...
**Every new carousel must include this visual call-to-action on Slide 1:**
...
This CTA should be placed in the top-right corner of Slide 1. It signals to the viewer that more context exists in the post caption/description. This is mandatory for all new carosséis generated starting from now.
...
|---|------|------------|---------|
| 1 | Hero | LIGHT_BG | Hook — bold statement, logo lockup, **"Leia a Descrição" CTA badge** (mandatory) |
...
| 7 | CTA | Brand gradient | Call to action — logo, tagline, **CTA button with "Save this for later"**. **No arrow. Full progress bar.** |

### 🚨 Mandatory CTA — Last Slide

**The last slide must include a clear call-to-action in Portuguese (pt-BR).** Recommended options:

| CTA Type | Text | When to Use |
|----------|------|-------------|
| Save | "👆 Salve para depois" | Default — drives Saves metric (highest for algorithm) |
| Follow | "👆 Siga para mais [tópico]" | When goal is follower growth |
| Share | "👆 Compartilhe com quem precisa" | When goal is organic reach |
| Comment | "👆 Comente abaixo com [pergunta]" | When goal is engagement |

The CTA button should use the format defined in the CTA button section above.
...
Apply the shared `layout-fit-policy` before approving any slide.

For this carousel system:

- overflow is a hard reject
- title length changes live body capacity
- `insight-grid` with more than 2 cards is unsafe by default
- `steps-list` depends on top weight and is not fixed-capacity
- stacked `metrics` blocks count as cards for fit purposes

Use these operating limits:

- `insight-grid`
  - safe default: max 2 cards
  - if the title is long, reduce content or switch template

- `steps-list`
  - `top-light`: up to 3 short steps
  - `top-medium`: up to 2 short steps
  - `top-heavy`: avoid this template or split the content

- `metrics-stack`
  - safe default: max 2 stacked metric blocks
  - treat metrics as cards in fit decisions
  - if the headline grows, 2 blocks may already be the limit

If content does not fit, use this correction order:

1. shorten support copy
2. simplify the component type
3. reduce cards or steps
4. split into more slides
5. switch template
6. only then tighten spacing or type size

Never start by shrinking the whole design.
...
**Fit note:** `steps-list` becomes high risk as soon as the headline pushes the
body downward. If the title is not clearly light, do not assume 3 steps fit.
...
- **Spacing system**: 8px base unit. All padding, gap and margin must be multiples of 8 (8, 16, 24, 32, 40, 48, 56, 64). No fractional or arbitrary values.
- Content padding: `0 32px` standard (4 × 8px)
- Bottom-aligned slides with progress bar: `0 32px 56px` to clear the bar (56 = 7 × 8px, progress bar is ~40px)
- **Hero/CTA slides:** `justify-content: center`
- **Content-heavy slides:** `justify-content: flex-end`
- **Content must never overlap the progress bar** — use `padding-bottom: 56px`
- Line heights must use unitless values (e.g. `1.5` not `24px`)

### Export Mode (mandatory CSS class)

All carousel HTML must include a `.export-mode` CSS block that hides chrome elements when set on `<body>`:
...
Add this block to every carousel's `<style>` section. The export script adds `export-mode` class to the body before capturing slides. This ensures exported PNG/JPEG files contain clean slide content without preview-only chrome (swipe arrows, progress bars, IG frame header, actions, caption).
...
3. Check whether any content is cut below the visible frame or compressed beyond safe readability
4. If a slide overflows, restructure it using the layout fit rules before making visual micro-adjustments
5. Fix only the mentioned slides — never regenerate the entire carousel unless the direction fundamentally changes
6. Only proceed to export when the user explicitly confirms approval (e.g., "pode exportar", "aprovado", "ok")
...
1. **Apply export-mode before capturing**: Add class `export-mode` to `<body>` and remove all chrome elements (swipe arrows, progress labels, IG header, dots, actions, caption) before taking screenshots. The CSS rule is mandatory in every carousel's `<style>` (see Export Mode section above).
2. **Use Python for HTML generation** — never use shell scripts with variable interpolation. Always use `Path.write_text()` or `open().write()`.
...
10. **Layout fit is a gate** — cropped or compressed slides are rejected before export
11. **Iterate fast** — show preview, fix specific slides, don't rebuild from scratch
