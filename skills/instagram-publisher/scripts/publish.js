...
async function main() {
  ...
  if (images.length > 10) {
    throw new Error(`Instagram posts support 1 image or carousels with up to 10 images (got ${images.length})`);
  }
  ...
  if (imageUrls.length === 1) {
    ...
    const imageContainerId = await createImageContainer(
      INSTAGRAM_USER_ID, imageUrls[0], caption, INSTAGRAM_ACCESS_TOKEN
    );
    await pollUntilFinished(imageContainerId, INSTAGRAM_ACCESS_TOKEN);
    console.log(`   Image container ID: ${imageContainerId}`);

    if (dryRun) {
      console.log('\n✅ DRY RUN complete — skipping final publish call.');
      console.log(`   Image container ready: ${imageContainerId}`);
      return;
    }

    console.log('\n🚀 Publishing to Instagram...');
    const postId = await publishMedia(INSTAGRAM_USER_ID, imageContainerId, INSTAGRAM_ACCESS_TOKEN);
  ...
  console.log('\n📦 Creating Instagram carousel child containers...');
